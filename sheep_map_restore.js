(() => {
  ...
  function buildCharactersByLocationIndex(rootData, activeName = '') {
    ...
    const 读取静态节点父级 = 节点名 => {
      ...
      const 访问 = (地点表, 父名 = '') => {
        ...
        for (const [当前名, 当前节点] of Object.entries(地点表)) {
          ...
          if (当前名 === 目标名 || 显示名 === 目标名) return { 名称: 当前名, 父名 };
    ...
    const 写入人物地点索引 = (地点名, 条目) => {
      ...
      charactersByLoc.get(安全地点名).push(条目);
    };
    safeEntries(charData).forEach(([charId, charInfo]) => {
      ...
      const 直接父节点名 = 动态命中名
        ? toText(动态地点表[动态命中名] && 动态地点表[动态命中名].归属父节点, '')
        : toText(静态命中 && 静态命中.父名, '') || (地点片段列表.length >= 2 ? 地点片段列表[地点片段列表.length - 2] : '');
...
请结合当前设施、在场角色与地点功能，自然写出这次行动的过程、收获与后续推进；若当前节点并不适合该动作，也请在剧情里明确说明阻碍原因。正文不要直接复述 JSONPatch 或系统术语。`;
  ...
  function renderMapInfoState() {
    ...
    const 可交互人物名列表 = characterEntries.filter(entry => entry && entry.可交互 !== false).map(entry => entry.name);
      ...
      : (inPreview && !previewCurrentBranch)
        ...
        : (!focusItem || !characterEntries.length)
          ...
          : `<div class="map-npc-roster-head"><span>${escapeMapHtml(在场人物节点文本)}</span><span>${escapeMapHtml(selectedNpc ? `已选 ${selectedNpc}` : `全部 ${characterEntries.length} 人`)}</span></div>${characterEntries.map(entry => {
            ...
            const 可交互人物 = entry && entry.可交互 !== false;
