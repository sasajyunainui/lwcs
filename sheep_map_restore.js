(() => {
  ...
  const 地图维护地点模板 = {
    place: { 标签: '普通地点', 类型: '动态地点', 节点类型: '地点', 交互: ['inspect'], 服务: [] },
    city: { 标签: '城市', 类型: '城市', 节点类型: '城市', 交互: ['inspect'], 服务: [], 默认商店: true },
    shop: { 标签: '商店', 类型: '商店', 节点类型: '商店', 交互: ['inspect', 'trade'], 服务: ['shop'], 默认商店: true },
    craft: { 标签: '工坊', 类型: '工坊', 节点类型: '工坊', 交互: ['inspect', 'craft'], 服务: ['craft'] },
    guild: { 标签: '协会', 类型: '协会', 节点类型: '协会', 交互: ['inspect', 'trade', 'craft'], 服务: ['shop', 'craft'], 默认商店: true },
    training: { 标签: '训练点', 类型: '训练设施', 节点类型: '训练', 交互: ['inspect', 'train_body', 'train_mind'], 服务: ['train'] },
    rest: { 标签: '休息点', 类型: '休息设施', 节点类型: '休息', 交互: ['inspect', 'rest', 'sleep', 'meditate'], 服务: ['rest'] },
  };
  ...
  function buildRoutineActionPreview(snapshot, action = '', 节点项 = null, durationTicks = 6) {
    ...
    if (safeAction === 'meditate') {
      ...
      let spMaxGain = (0.2 + coreCount * 0.2) * (持续tick / 6) * getCultivationTalentRate(stat.天赋梯队) * multiplier;
  ...
  function 构建地图维护地点数据(输入 = {}) {
    ...
    const 交互集合 = new Set(模板.交互 || ['inspect']);
    const 服务集合 = new Set(模板.服务 || []);
    if (输入.启用商店) {
      交互集合.add('trade');
      服务集合.add('shop');
    }
    ...
    const 新地点 = {
      ...
      y: 坐标.y,
      归属父节点: 获取地图维护父节点名(),
      地图ID: toText(mapState.currentMapId, 'map_douluo_world'),
      交互: Array.from(交互集合),
      服务: Array.from(服务集合)
    };
  ...
  function 打开地图维护弹窗(操作 = '', 上下文 = {}) {
    return new Promise(resolve => {
      ...
      弹窗.addEventListener('click', event => {
      ...
      if (类型选择) {
        类型选择.addEventListener('change', 刷新附加选项);
        刷新附加选项();
      }
      表单.addEventListener('submit', event => {
...
请结合当前设施、在场角色与地点功能，自然写出这次行动的过程、收获与后续推进；若当前节点并不适合该动作，也请在剧情里明确说明阻碍原因。正文不要直接复述 JSONPatch 或系统术语。`;
  ...
  function renderMapInfoState() {
    ...
    if (allowLocalActions && !canPreviewEnter) {
      ...
      const 可结算地点行动 = new Set(['trade', 'bid', 'craft', 'battle', 'meditate', 'rest', 'sleep', 'train', 'train_body', 'train_mind']);
      const 加入地点行动 = 动作 => {
        const 标准动作 = toText(动作, '');
        ...
        else if (可结算地点行动.has(标准动作)) 地点行动集合.add(标准动作);
      };
      const 显式地点行动 = []
      ...
      if (节点类型 === 'commerce') {
        ...
        if (/静室|冥想|修炼|修炼室/.test(`${focusName} ${focusTypeDisplay}`)) 加入地点行动('meditate');
      } else if (节点类型 === 'rest') {
        加入地点行动('rest');
        加入地点行动('meditate');
      }
    ...
    getMapUiElements('[data-map-duration-select]').forEach(选择框 => {
      ...
      if (选择框.value !== String(当前日常tick)) {
        ...
        自定义选项.value = String(当前日常tick);
        自定义选项.textContent = 格式化地图日常时长(当前日常tick);
    ...
    if (isSubMapView && focusItem && !isFreeSelection) {
      ...
      const 商品预览HTML = 商店上下文.商店名 && 商店上下文.商品列表.length
      ...
      setMapHtml('[data-map-npc-list]', `${动作预览HTML}${商品预览HTML}`);
    } else {
  ...
  function 打开地图维护右键菜单(事件, 选项 = {}) {
    ...
    document.addEventListener('pointerdown', 关闭地图维护右键菜单._关闭点击, true);
    document.addEventListener('keydown', 关闭地图维护右键菜单._关闭按键, true);
  }
  ...
  function ensureMapInteractionBindings() {
    ...
    getMapUiElements('.map-canvas.interactive-map').forEach(canvasEl => {
      ...
      canvasEl.addEventListener('contextmenu', handleMapCanvasContextMenu);
    });
    ...
    getMapUiElements('[data-map-node-layer]').forEach(layerEl => {
      ...
      layerEl.addEventListener('contextmenu', handleNodeLayerContextMenu);
    });
