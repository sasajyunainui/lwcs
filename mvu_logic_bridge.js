    ...
    function buildTerminalHeroCard(snapshot) {
      ...
      const pendingRequestText = snapshot.pendingRequests[0] || '当前没有安排';
    ...
    function buildLiveArchiveModal(previewKey) {
      ...
      if (previewKey === '全息星图主画布') {
        ...
        const patchCards = (snapshot.mapActivePatchEntries.length ? snapshot.mapActivePatchEntries : [['暂无激活补丁', { empty: true }]]).map(([name, patch]) => ({
          ...
          desc: patch && patch.empty ? '当前地图没有激活中的补丁。' : `当前存在激活补丁 ｜ 区域 ${toText(deepGet(patch, 'bounds.x', 0), 0)},${toText(deepGet(patch, 'bounds.y', 0), 0)}`
        }));
              ...
              <div class="archive-modal-grid">
                ...
                <div class="archive-card full">
                  ...
                  <div class="tag-cloud armory-quick-actions">
                    ${mechExists ? `<button type="button" class="tag-chip live armory-action-btn" data-armory-action="equip_mech">装备机甲</button><button type="button" class="tag-chip warn armory-action-btn" data-armory-action="unequip_mech">卸下机甲</button>` : '<span class="tag-chip">当前没有可用机甲</span>'}
                ...
                <div class="archive-card full">
                  ...
                  ${makeTimelineStack(
                    accessoryEntries.length
                      ...
                      : [{ title: '当前没有附件', desc: 'MVU.txt 中 accessories 为杂项记录表。' }]
                  )}
                </div>
                <div class="archive-card full">
          ...
          body: `
            <div class="equipment-layout">
              ...
              <div class="archive-card">
                ...
                <div class="tag-cloud armory-quick-actions">${quickActionButtons || '<span class="tag-chip">当前没有可操作装备</span>'}</div>
              </div>
              <div class="archive-card full">
            ...
            <div class="archive-modal-grid" style="grid-template-columns: 1fr;">
              <div class="archive-card full">
                ...
                ${makeTimelineStack([
                  ...
                  { title: '安排摘要', desc: snapshot.pendingRequests[0] || '当前没有安排' },
...
${JSON.stringify(patchOps, null, 2)}
    ...
    document.addEventListener('click', (event) => {
      ...
      const inVueShell = (leftMount && leftMount.contains(clickable))
        ...
        || !!clickable.closest('.mvu-vue-wrapper');
