...
const SKILL_PROTOTYPE_FIELD_OPTIONS_V1 = Object.freeze({
  ...
  判断: Object.freeze(['混乱', '误判', '判断干扰', '判断抵抗', '感知', '自身随机波动', '概率偏移', '厄运反噬']),
...
const SKILL_PROTOTYPE_DEFINITION_LIST_V1 = Object.freeze([
  ['伤害结算', '战斗结算', '敌对', ['威力倍率', '伤害类型', '攻击段数', '防御穿透', '触发', '持续回合'], ['威力倍率', '伤害类型'], '造成直接、持续、多段或延迟伤害'],
  ...
  ['结算修正', '战斗结算', '上下文', ['结算', '数值', '持续回合', '驱动属性', '影响方向'], ['结算', '数值'], '修正最终伤害、治疗、护盾、消耗、前摇等结果'],
  ...
  ['规则改写', '战斗结算', '上下文', ['规则', '数值', '改写为', '驱动属性', '影响方向'], ['规则'], '改写治疗、护盾、伤害、目标、消耗等规则'],
...
function 收口执行条件分支条目_V1(value = {}, recordViolation = () => {}, parentEffect = {}) {
  ...
  if (处理 === '替换效果' || 处理 === '追加效果') {
    ...
    const 效果列表 = 源列表
      .flatMap(effect => 收口执行效果条目列表_V1(effect, parentEffect.目标 || '单体', recordViolation))
      .filter(Boolean);
    if (!效果列表.length) {
...
function 补足机制模板原型数值_V1(原型 = '', 字段 = {}, source = {}) {
  ...
  if (原型 === '判定修正') {
    ...
    if (字段.状态 === undefined) 字段.状态 = String(source.状态 || source.状态类型 || '任意负面').trim() || '任意负面';
  ...
  if (directionTag === '锁定') {
    return [
      ...
      },
      {
        ...
        目标: targetModel,
        持续回合: duration,
        增加命中: Number(({ C: 0.06, B: 0.1, A: 0.14, S: 0.18 }[grade] || 0.1).toFixed(2)),
        锁定层级: grade === 'S' ? 3 : grade === 'A' ? 2 : 1,
        驱动属性: '敏捷',
        影响方向: '成功率',
      },
    ];
...
function buildSkillCombatProfile(blueprint, qualityCtx = {}) {
  ...
  const 战斗 = {
    ...
    消耗: '无',
    仲裁逻辑: {
      瞬间交锋模块: { 基础威力倍率: 0, 伤害类型: '无', 防御穿透: 0, 护盾绝对值: 0, 瞬间恢复比例: 0 },
      状态挂载模块: {
        状态名称: '无',
        持续回合: 0,
...
function shouldWrapSkillEffectAsGrant(effect = {}) {
  ...
  if (!mechanism || ['系统基础', '机制授予'].includes(mechanism)) return false;
  if (
    [
      '属性变化',
      '持续恢复',
...
function buildSingleSkillEffectSummary(effect) {
  ...
  switch (机甲) {
    ...
    case '条件触发':
      return '满足[' + String(effect.触发条件 || effect.驱动属性 || '条件') + ']时触发预设效果';
    case '规则改写':
  ...
  if (main === '特殊规则类' && archetype === '条件触发')
    packedEffects.push(
      ...
      },
      {
        ...
        目标: normalizedTarget,
        结算: '最终伤害',
        数值: 格式化原型比例变化_V1(Number(state.计算层效果?.final_damage_mult || 1.18), 1, true),
        驱动属性: '体力上限',
        影响方向: '效果强度',
      },
      {
        ...
        目标: normalizedTarget,
        判定: '命中',
        数值: 格式化原型比例变化_V1(Number(state.计算层效果?.hit_bonus || 0), 1),
      },
    );
