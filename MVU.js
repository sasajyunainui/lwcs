...
import { TimelineEvents } from 'https://cdn.jsdelivr.net/gh/sasajyunainui/lwcs@V1.35/timeline.js';
...
function generateBackgroundNPCs(data) {
  ...
  const nonHumanFactions = ['深渊位面', '魂兽一族', '星斗大森林', '极北之地', '无尽海域', '自然地带'];

  const factionSpiritMap = {
    昊天宗: { name: '昊天锤', type: '强攻系' },
    蓝电霸王龙家族: { name: '蓝电霸王龙', type: '强攻系' },
    本体宗: { name: '本体武魂', type: '强攻系' },
    泰坦巨猿家族: { name: '泰坦巨猿', type: '强攻系' },
    神圣天使家族: { name: '神圣天使', type: '强攻系' },
  };
  ...
  _(data.org).forEach((orgData, orgName) => {
    if (!orgData?.power_stats || typeof orgData.power_stats !== 'object') return;
    if (nonHumanFactions.includes(orgName)) return;

    const currentCounts = { limit: 0, super: 0, title: 0 };
    _(data.char).forEach((c, cName) => {
      if (c.status?.alive && c.social?.factions?.[orgName]) {
        const lv = c.stat?.lv || 0;
    ...
    const targetStats = {
      limit_douluo: Number(orgData.power_stats.limit_douluo || 0),
      super_douluo: Number(orgData.power_stats.super_douluo || 0),
      title_douluo: Number(orgData.power_stats.title_douluo || 0),
    };
    const toGenerate = [
    ...
    toGenerate.forEach(tier => {
      for (let i = 0; i < tier.count; i++) {
        const newName = getRandomName(orgName);
        const genLv = Math.floor(Math.random() * (tier.maxLv - tier.minLv + 1)) + tier.minLv;

        let spiritName = '未知武魂';
        let spiritType = '强攻系';
        let spiritObj = {};
        const eliteLv99Types = ['强攻系', '控制系', '敏攻系'];

        if (factionSpiritMap[orgName]) {
          spiritName = factionSpiritMap[orgName].name;
          spiritType = factionSpiritMap[orgName].type;
          spiritObj = {
            [spiritName]: { 表象名称: spiritName, type: spiritType, soul_spirits: {} },
          };
        } else {
          const types = ['强攻系', '敏攻系', '控制系', '防御系', '辅助系', '治疗系', '精神系', '元素系'];
          spiritType = types[Math.floor(Math.random() * types.length)];
        }

        if (genLv >= 99 && !eliteLv99Types.includes(spiritType)) {
          spiritType = eliteLv99Types[Math.floor(Math.random() * eliteLv99Types.length)];
          if (spiritObj[spiritName]) spiritObj[spiritName].type = spiritType;
        }

        data.char[newName] = CharacterSchema.parse({
          stat: {
            lv: genLv,
            age: 80 + Math.floor(Math.random() * 50),
            talent_tier: genLv >= 95 ? '顶级天才' : '天才',
            type: spiritType,
          },
          spirit: spiritObj,
          social: {
...
function autoBreakthrough(data) {
  _(data.char).forEach((c, charName) => {
    ...
    const isBeast = c.stat.age >= 10000 || c.social?.factions?.['魂兽一族'];
    if (isBeast) return;
...
const SKILL_MAIN_MECHANIC_POOL_V1 = {
  ...
  特殊规则类: {
    ...
    children: ['复制', '反制', '转化', '状态交换', '强制绑定/锁定', '条件触发', '规则改写'],
  },
};
...
const SKILL_ARCHETYPE_POOL_V1 = {
  ...
  特殊规则类: ['复制', '反制', '转化', '状态交换', '强制绑定/锁定', '条件触发', '规则改写'],
};
...
function rollSubModelByGrade(mainMechanic, grade, roll) {
  const tables = {
    ...
    特殊规则类: {
      C: [{ value: '条件触发', weight: 100 }],
      B: [
        { value: '条件触发', weight: 40 },
      ...
      A: [
        ...
        { value: '反制', weight: 20 },
        { value: '转化', weight: 15 },
        { value: '状态交换', weight: 15 },
        { value: '强制绑定/锁定', weight: 15 },
        { value: '条件触发', weight: 10 },
...
function rollSkillBlueprint(type, grade, ringIndex, preferredSecondary = []) {
  ...
  const delivery = type === '食物系' ? '造物承载' : pickRandom(deliveryPool) || '直接生效';
...
function buildSkillBattleSummary(blueprint) {
  ...
  if (['持续伤害', '持续恢复', '范围展开'].includes(archetype) || ['延迟触发', '范围展开'].includes(delivery))
    summary.持续性 = '中';
  ...
  if (
    ...
    summary.协同性 = '高';
  else if (['单属性增益', '感知干扰', '反制'].includes(archetype)) summary.协同性 = '中';
...
function buildSkillCombatProfile(blueprint, qualityCtx = {}) {
  ...
  if (main === '伤害类') {
    ...
    if (archetype === '霸体') state.特殊机制标识 = '霸体';
    if (archetype === '免死/锁血') {
      state.特殊机制标识 = '免死';
...
function createNumericStatBonusMap(seed = {}) {
  return {
    str: Number(seed?.str || 0),
    def: Number(seed?.def || 0),
    agi: Number(seed?.agi || 0),
    vit_max: Number(seed?.vit_max || 0),
    men_max: Number(seed?.men_max || 0),
    sp_max: Number(seed?.sp_max || 0),
  };
}
...
function autoGenerateSkill(
  ...
  if (main === '防御类' && archetype === '霸体')
    packedEffects.push({ 机制: '霸体', 目标: normalizedTarget, 持续回合: state.持续回合 || 1 });
...
const AI_TODO_ATTRIBUTE_CAPACITY = '待补全（填写可容纳属性列表；无属性体系请填“无”）';
...
const AppearanceSchema = z
  .object({
    ...
    特殊特征: z
      .array(z.string())
      .prefault(['待补全(根据角色辨识度补全特殊特征，如伤疤/胎记/神态)'])
  ...
  .prefault({
    ...
    身高: '待补全(根据角色设定补全身高)',
    体型: '待补全(根据角色体态补全体型)',
    特殊特征: ['待补全(根据角色辨识度补全特殊特征，如伤疤/胎记/神态)'],
  });
    ...
    travel_request: z
      .object({
        ...
        method: z
          ...
          .prefault('步行')
          .describe(
            '交通方式：步行/校园短驳车/魂导汽车/魂导列车/远洋巨轮/飞行(机甲/斗铠)/空间传送(极限斗罗)/空间传送(神级)',
          ),
      })
      .prefault({}),
    promotion_request: z
...
export const Schema = z
  ...
  .transform(data => {
    ...
    data.world._upcoming_timeline_preview_text = buildUpcomingTimelinePreviewText(upcomingTimelinePreview);
    ...
    if (!data.world.flags['initial_setup_complete']) {
      ...
      if (data.sys.rsn === '初始化' || !data.sys.rsn) data.sys.rsn = '';
      data.sys.rsn += ' [系统提示] 世界初始化完成！背景强者已生成，大陆风云榜与少年天才榜已完成首次排位！';
    }
      ...
      if (c.status.consuming_herb_age > 0) {
        ...
        if (c.stat.lv_penalty > 0 && age >= 10000) {
          ...
          if (c.stat.lv - c.stat.last_herb_lv >= 20) {
            ...
            c.stat.trained_bonus.sp_max += gain;
    ...
    _(data.char).forEach((c, charName) => {
      if (c.travel_request && (c.travel_request.target_loc !== '无' || c.travel_request.target_x !== -1)) {
        ...
        if (req.target_loc !== '无') {
          ...
          } else if (data.world.dynamic_locations && data.world.dynamic_locations[req.target_loc]) {
            let dynLoc = data.world.dynamic_locations[req.target_loc];
            if (dynLoc.x !== undefined) {
              endX = dynLoc.x;
              endY = dynLoc.y;
            } else if (FLAT_LOCATIONS[dynLoc.归属父节点]) {
              endX = FLAT_LOCATIONS[dynLoc.归属父节点].x;
              endY = FLAT_LOCATIONS[dynLoc.归属父节点].y;
            }
          }
        } else {
        ...
        if (startX === endX && startY === endY) {
          ...
          if (!canTravel) {
            data.sys.rsn = `[跑图失败] ${charName}${travelCost.reason ? `：${travelCost.reason}` : ''}，无法使用【${req.method}】前往【${finalLocName}】！`;
          } else {
            ...
            if (travelCost.vit > 0) {
              ...
              if (c.stat.vit <= 1)
                c.stat.conditions['疲劳'] = { 类型: 'debuff', 层数: 1, 描述: '长途跋涉导致极度疲劳' };
            }
          }

          if (canTravel) {
            data.world.time.tick += costTicks;
            c.status.loc = finalLocName;
            c.status.current_x = endX;
            c.status.current_y = endY;
            c.status.coord_system =
              isWorldLocationName(finalLocName, data) || req.target_loc === '无'
                ? MAP_COORD_SYSTEM_IMAGE
                : MAP_COORD_SYSTEM_LOCAL;
    ...
    _(data.char).forEach((c, charName) => {
      ...
      if (c.donate_request && c.donate_request.item_name !== '无') {
        ...
        if (!c.inventory[itemName] || c.inventory[itemName].数量 < req.quantity) {
          ...
          data.sys.rsn = `[物资捐献] ${charName} 向【${req.target_faction}】上交了 ${req.quantity} 份【${itemName}】。经天赋系数(${talentMult})加成，获得 ${pointsEarned} 贡献点！`;
        }
    ...
    if (data && data.char) {
        ...
        _(sourceChar.spirit || {}).forEach((spiritData, spiritKey) => {
          ...
          _(spiritData?.soul_spirits || {}).forEach((soulSpirit, slotName) => {
            ...
            spiritCombatSummary.push({
              ...
              对标等级: Number(soulSpirit.战力面板?.对标等级 || 0),
      ...
      const filtered_world = {
        ...
        _upcoming_timeline_preview_text: data.world?._upcoming_timeline_preview_text || '',
        locations: {},
        dynamic_locations: {},
      };
