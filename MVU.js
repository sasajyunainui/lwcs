...
const AI_TODO_SOUL_BONE_SOURCE = '待生成(请填写魂骨来源的魂兽名)';
...
function isStorageTodoPlaceholderText(value) {
  ...
  if (/^待生成\(名称格式：魂兽名\+.+魂骨/.test(text)) return false;
...
function 构建魂骨名称待生成提示词_V1(骨部位 = '魂骨') {
  ...
  return `待生成(名称格式：魂兽名+${安全部位}，例如暗金恐爪熊${安全部位})`;
}
...
function 构建机制待生成提示词_V1(字段名 = '主机制', 候选列表 = [], 附加规则 = '') {
  ...
  return `待生成（请结合武魂特性、魂环来源魂兽特性与当前剧情上下文，从候选中选择最合适的${名称}并写回候选原文；同时与魂技名、画面描述保持一致；禁止输出候选外内容。候选：${候选文本}${规则文本 ? `；${规则文本}` : ''}）`;
}

function 构建多方向待生成提示词_V1() {
  return '待生成（请结合武魂特性、魂环来源魂兽特性与当前剧情上下文，并与魂技名、画面描述保持一致，在“是/否”中二选一；若该技能应具备两个及以上并行效果分支则填“是”，否则填“否”）';
}

function 构建多方向类型待生成提示词_V1(候选列表 = []) {
  const 候选文本 = 规范化机制枚举数组_V1(候选列表).join('、') || '无';
  return `待生成（请结合武魂特性、魂环来源魂兽特性与当前剧情上下文，并与魂技名、画面描述和是否多方向保持一致，从候选中选择最合适的多方向类型并写回候选原文；若是否多方向=否则填写“无”。候选：${候选文本}）`;
}

function 规范化机制限选单值_V1(值 = '', 候选列表 = [], 默认值 = '待生成') {
  const 候选项 = 规范化机制枚举数组_V1(候选列表);
  if (!候选项.length) return 默认值;
  const 文本 = String(值 || '').trim();
  if (!文本 || /^待生成/.test(文本)) return 默认值;
...
function 是否机制待生成文本_V1(字段值 = '') {
  return /^待生成/.test(String(字段值 || '').trim());
}
...
function 构建技能机制决策临时数据_V1(skill = {}, context = {}) {
  ...
  机制选择.变异机制 = 规范化机制限选多值_V1(原机制选择.变异机制, SKILL_MUTATION_MECHANIC_TYPES_V1);
  if (需要副作用字段) {
...
const SkillStructSchema = z
  .object({
    ...
    机制决策临时: (() => {
      const 枚举回退 = (候选列表 = [], 回退值 = '待生成') => {
        const 候选项 = 规范化机制枚举数组_V1(候选列表);
        return 候选项.length ? 候选项 : [回退值];
      };
      const 机制候选池 = 构建技能机制候选池_V1();
      const 主机制枚举 = 枚举回退(机制候选池.主机制候选, '待生成');
      const 副机制枚举 = 枚举回退(机制候选池.副机制候选, '待生成');
      ...
      return z
        .object({
          机制选择: z
            .object({
              主机制: z.union([z.enum(主机制枚举), z.literal('待生成'), z.literal(主机制待生成提示词)]).prefault(主机制待生成提示词),
              副机制1: z.union([z.enum(副机制枚举), z.literal('待生成'), z.literal(副机制1待生成提示词)]).optional(),
              副机制2: z.union([z.enum(副机制枚举), z.literal('待生成'), z.literal(副机制2待生成提示词)]).optional(),
              变异机制: z.array(z.enum(变异机制枚举)).prefault([]),
              副作用类型: z.union([z.enum(副作用枚举), z.literal('待生成'), z.literal(副作用待生成提示词)]).optional(),
              是否多方向: z.union([z.enum(['是', '否']), z.literal('待生成'), z.literal(多方向待生成提示词)]).prefault(多方向待生成提示词),
              多方向类型: z.union([z.enum(多方向类型枚举), z.literal('待生成'), z.literal(多方向类型待生成提示词)]).prefault(多方向类型待生成提示词),
            })
            .prefault({}),
          对象差异规则: 对象差异规则临时字段,
        })
