...
const SKILL_ARCHETYPE_POOL_V1 = {
  ...
  特殊规则类: ['召唤', '分身', '复制', '反制', '转化', '状态交换', '状态转移', '强制绑定/锁定', '条件触发', '规则改写', '重力倍率调整', '引爆持续伤害', '斩盾', '窃取护盾', '吞噬', '能力共享', '机制抹消', '机制窃取', '炸环', '时光回溯', '气运干涉', '厄运反噬'],
};
...
const SKILL_SECONDARY_BY_MAIN_V1 = {
  ...
  削弱类: ['禁疗', '减速', '迟缓', '标记弱点', '缴械', '驱散增益', '破隐', '治疗反转', '封技', '元素封禁', '斩盾', '吞噬', '机制抹消'],
  ...
  特殊规则类: ['共享视野', '标记弱点', '净化', '驱散增益', '窃取增益', '隐身', '护卫', '状态转移', '重力倍率调整', '引爆持续伤害', '斩盾', '窃取护盾', '吞噬', '能力共享', '机制抹消', '机制窃取', '炸环', '时光回溯', '气运干涉', '厄运反噬', '规则改写', '治疗反转', '封技', '无敌金身', '伤害反射', '伤害分摊', '替身抵消', '复苏'],
};
...
const SKILL_MECHANISM_META_V1 = (() => {
  ...
  register('复制', {
    ...
    说明: '复制目标技能、属性，或同时复制目标全部可用技能与属性；不复制领域、真身、炸环代价链或限制次数。',
    designerMainHint: '特殊规则类',
    designerSubHint: '复制',
    设计台参数定义: [
...
const 技能分支判定结果候选_V1 = Object.freeze(['是', '否', '未知']);
...
function 技能机制满足品质门槛_V1(机制名 = '', context = {}) {
  const 门槛 = normalizeSkillGradeSymbol(技能机制最低品质_V1[String(机制名 || '').trim()] || 'F');
...
function 构建技能机制候选池_V1(context = {}) {
  ...
  Object.entries(SKILL_MECHANISM_META_V1 || {}).forEach(([机制名, 元数据]) => {
    ...
    if (!name) return;
    if (!技能机制满足品质门槛_V1(name, context)) return;
...
function 构建分支判定待生成提示词_V1() {
  return '待补全（仅当是否多方向=是时填写；条目可为空数组。每条格式：{分支标记:"分支1",条件说明:"一句话写明非数值/剧情条件",是否满足:"是/否/未知"}。只判断脚本无法稳定计算的剧情前置、血脉、性别、年龄、身份、环境等；血量/资源/命中/等级差等战斗中会变化的条件不要写入这里）';
}

function 规范化技能分支判定条目_V1(value = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const 分支标记 = String(value.分支标记 || value.branch || value.id || '').trim();
  if (!分支标记) return null;
  const 条件说明 = String(value.条件说明 || value.条件 || value.condition || '').trim();
  const rawResult = String(value.是否满足 || value.满足 || value.result || '').trim();
  const 是否满足 = 技能分支判定结果候选_V1.includes(rawResult) ? rawResult : '未知';
  return { 分支标记, 条件说明, 是否满足 };
}

function 规范化技能分支判定条目列表_V1(value = []) {
  const source = Array.isArray(value) ? value : [];
  return source.map(item => 规范化技能分支判定条目_V1(item)).filter(Boolean);
}

function 读取技能分支判定映射_V1(决策数据 = {}) {
  const 分支判定 = 决策数据?.分支判定 && typeof 决策数据.分支判定 === 'object' && !Array.isArray(决策数据.分支判定)
    ? 决策数据.分支判定
    : {};
  const 条目列表 = 规范化技能分支判定条目列表_V1(分支判定.条目 || []);
  const 映射 = new Map();
  条目列表.forEach(item => {
    if (!映射.has(item.分支标记)) 映射.set(item.分支标记, item);
  });
  return 映射;
}
...
function 构建技能机制决策临时数据_V1(skill = {}, context = {}) {
  ...
  const 分支判定待生成提示词 = 构建分支判定待生成提示词_V1();
  ...
  对象差异规则.条目 = Array.isArray(对象差异规则.条目)
    ...
    : [];
  const 分支判定 = 现有决策 && typeof 现有决策.分支判定 === 'object' && !Array.isArray(现有决策.分支判定)
    ? cloneJsonValue(现有决策.分支判定)
    : {};
  const 分支旧提示词 = String(分支判定.填写提示词 || '').trim();
  分支判定.填写提示词 = /^待补全/.test(分支旧提示词) ? 分支旧提示词 : 分支判定待生成提示词;
  分支判定.条目 = 规范化技能分支判定条目列表_V1(分支判定.条目 || []);
  return {
    机制选择,
    对象差异规则,
    分支判定,
...
function 按候选方案回填技能效果数组_V1(skill = {}, context = {}) {
  ...
  const 分支判定映射 = 读取技能分支判定映射_V1(决策数据);
  if (分支判定映射.size > 0) {
    效果数组.forEach(effect => {
      const 分支标记 = String(effect?.分支标记 || '').trim();
      if (!分支标记 || !分支判定映射.has(分支标记)) return;
      const 判定条目 = 分支判定映射.get(分支标记);
      if (判定条目.条件说明) effect.分支条件说明 = 判定条目.条件说明;
      effect.分支判定结果 = 判定条目.是否满足 || '未知';
    });
  }
      ...
      return z
        .object({
          ...
          分支判定: z
            .object({
              填写提示词: z
                .union([z.literal('待补全'), z.literal(分支判定待生成提示词)])
                .prefault(分支判定待生成提示词),
              条目: z
                .array(
                  z.object({
                    分支标记: z.string().prefault('分支1'),
                    条件说明: z.string().prefault(''),
                    是否满足: z.enum(技能分支判定结果候选_V1).prefault('未知'),
                  }),
                )
                .prefault([]),
            })
