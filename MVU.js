...
const SKILL_ARCHETYPE_POOL_V1 = {
  ...
  特殊规则类: ['分身', '复制', '反制', '转化', '状态交换', '状态转移', '强制绑定/锁定', '条件触发', '规则改写', '引爆持续伤害', '斩盾', '窃取护盾', '资源夺取', '资源反灌', '复活封锁'],
};
...
const SKILL_SECONDARY_BY_MAIN_V1 = {
  伤害类: ['穿透', '吸血', '斩杀补伤', '流血DOT', '打断', '反击', '追击', '引爆持续伤害', '斩盾', '资源夺取'],
  控制类: ['打断', '沉默', '减速', '致盲', '迟缓', '禁疗', '缴械', '嘲讽', '破隐', '封技'],
  削弱类: ['禁疗', '减速', '迟缓', '标记弱点', '缴械', '驱散增益', '破隐', '治疗反转', '封技', '斩盾', '资源夺取', '复活封锁'],
  增益类: ['小护盾', '净化', '解控', '共享视野', '隐身', '护卫', '无敌金身', '复苏', '资源反灌'],
  防御类: ['小护盾', '反击', '净化', '解控', '护卫', '嘲讽', '无敌金身', '伤害反射', '伤害分摊', '替身抵消', '复苏'],
  回复类: ['净化', '解控', '小护盾', '魂力恢复', '精神恢复', '驱散增益', '护卫', '复苏', '资源反灌'],
  '感知/认知类': ['标记弱点', '共享视野', '目标锁定', '打断', '沉默', '缴械', '驱散增益', '窃取增益', '破隐'],
  位移类: ['打断', '反击', '标记弱点', '缴械', '隐身', '破隐'],
  特殊规则类: ['共享视野', '标记弱点', '净化', '驱散增益', '窃取增益', '隐身', '护卫', '状态转移', '引爆持续伤害', '斩盾', '窃取护盾', '资源夺取', '资源反灌', '复活封锁', '治疗反转', '封技', '无敌金身', '伤害反射', '伤害分摊', '替身抵消', '复苏'],
};
...
const SKILL_SECONDARY_TYPE_BIAS_V1 = {
  ...
  控制系: ['封技', '治疗反转', '目标锁定', '缴械', '驱散增益', '嘲讽', '伤害分摊', '资源夺取', '复活封锁'],
  精神系: ['状态转移', '封技', '治疗反转', '窃取增益', '目标锁定', '隐身', '资源夺取', '资源反灌', '复活封锁'],
  元素系: ['引爆持续伤害', '斩盾', '封技', '治疗反转', '驱散增益', '破隐', '资源夺取', '复活封锁'],
  辅助系: ['护卫', '复苏', '共享视野', '驱散增益', '窃取护盾', '无敌金身', '伤害分摊', '资源反灌'],
  治疗系: ['复苏', '护卫', '驱散增益', '共享视野', '无敌金身', '窃取护盾', '伤害分摊', '资源反灌'],
  食物系: ['复苏', '驱散增益', '共享视野', '窃取护盾', '治疗反转', '护卫', '资源反灌'],
};
...
const SKILL_MECHANISM_META_V1 = (() => {
  ...
  register('资源夺取', {
    ...
    摘要提示: { skillType: '控制', mainType: '特殊规则类', controlStrength: '软控' },
    designerMainHint: '特殊规则类',
    designerSubHint: '资源夺取',
    designerSecondaryHint: '资源夺取',
    设计台参数定义: [
      select('resourceType', '夺取资源', 'RESOURCE_TRANSFER_TYPE'),
      num('drainRatio', '夺取比例', '0.18'),
  ...
  register('资源反灌', {
    ...
    designerMainHint: '特殊规则类',
    designerSubHint: '资源反灌',
    designerSecondaryHint: '资源反灌',
    设计台参数定义: [
      select('resourceType', '反灌资源', 'RESOURCE_TRANSFER_TYPE'),
      num('refeedRatio', '反灌比例', '0.2'),
    ],
  });
  register(['反复苏', '复活封锁'], {
    可主机制: true,
    ...
    designerMainHint: '特殊规则类',
    designerSubHint: '复活封锁',
    designerSecondaryHint: '复活封锁',
    设计台参数定义: [
      num('duration', '持续回合', '2', '1'),
    ],
  });
...
function buildSpecialRuleArchetypeWeightedTableByContext(grade, type = '强攻系') {
  ...
  if (type === '精神系') {
    ...
    table = rebalanceWeightedTableWithPreferredValue(table, '资源夺取', { C: 12, B: 18, A: 26, S: 34 }[grade] || 18);
  } else if (type === '辅助系' || type === '治疗系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '窃取护盾', { C: 10, B: 16, A: 24, S: 30 }[grade] || 16);
    table = rebalanceWeightedTableWithPreferredValue(table, '资源反灌', { C: 14, B: 22, A: 32, S: 42 }[grade] || 22);
  } else if (type === '精神系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '资源夺取', { C: 14, B: 22, A: 32, S: 44 }[grade] || 22);
    table = rebalanceWeightedTableWithPreferredValue(table, '资源反灌', { C: 10, B: 16, A: 24, S: 34 }[grade] || 16);
    table = rebalanceWeightedTableWithPreferredValue(table, '复活封锁', { C: 12, B: 20, A: 30, S: 40 }[grade] || 20);
  } else if (type === '控制系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '复活封锁', { C: 14, B: 22, A: 34, S: 46 }[grade] || 22);
  } else if (type === '食物系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '资源反灌', { C: 12, B: 18, A: 28, S: 36 }[grade] || 18);
  }
  return table;
}
...
function buildSkillBattleSummary(blueprint) {
  ...
  if (['硬控'].includes(archetype)) summary.控制强度 = '硬控';
    ...
    ['软控', '位移限制', '节奏打断', '前摇拉长', '掌控压制', '感知干扰', '幻境', '催眠', '认知扭曲', '资源夺取', '复活封锁'].includes(
      archetype,
    ) ||
  ...
  else if (['资源反灌'].includes(archetype)) summary.回复性质 = '资源回复';
  ...
  if (
    ...
    secondary.some(s => ['标记弱点', '共享视野', '目标锁定', '护卫', '资源反灌'].includes(s))
  )
    summary.协同性 = '高';
  else if (
    ['单属性增益', '感知干扰', '反制', '分身', '隐身', '资源反灌'].includes(archetype) ||
    secondary.some(s => ['窃取增益', '隐身', '资源夺取'].includes(s))
  )
    summary.协同性 = '中';
  ...
  summary.风险等级 = '中';
  if (['免死/锁血', '规则改写', '复制', '反制', '资源夺取'].includes(archetype)) summary.风险等级 = '高';
...
function buildSkillCombatProfile(blueprint, qualityCtx = {}) {
  ...
  if (main === '伤害类') {
    ...
    if (archetype === '资源夺取') {
      战斗.技能类型 = '控制';
      state.持续回合 = 0;
      state.特殊机制标识 = [state.特殊机制标识, '资源夺取'].filter(v => v && v !== '无').join('/') || '资源夺取';
    }
    if (archetype === '资源反灌') {
      战斗.技能类型 = '辅助';
      state.持续回合 = 0;
      state.特殊机制标识 = [state.特殊机制标识, '资源反灌'].filter(v => v && v !== '无').join('/') || '资源反灌';
    }
    if (archetype === '复活封锁') {
      战斗.技能类型 = '控制';
      state.持续回合 = { C: 1, B: 2, A: 2, S: 3 }[grade] || 2;
      state.特殊机制标识 = [state.特殊机制标识, '复活封锁'].filter(v => v && v !== '无').join('/') || '复活封锁';
    }
    if (archetype === '复制') {
      战斗.技能类型 = '辅助';
      state.持续回合 = { C: 1, B: 2, A: 2, S: 3 }[grade] || 2;
...
function buildSingleSkillEffectSummary(effect) {
  ...
  switch (机甲) {
    ...
    case '复活封锁':
    case '反复苏':
      return '封锁' + target + '的复苏回路，持续' + formatSkillNumber(effect.持续回合) + '回合';
    case '感知干扰':
...
const SKILL_SECONDARY_MECHANIC_TYPES_V1 = {
  伤害附加: ['穿透', '吸血', '斩杀补伤', '流血DOT', '追击', '反击', '引爆持续伤害', '斩盾', '资源夺取'],
  控制附加: ['打断', '沉默', '减速', '致盲', '迟缓', '禁疗', '缴械', '封技'],
  防御附加: ['小护盾', '短减伤', '反伤', '霸体短附加', '护卫', '嘲讽', '无敌金身', '伤害反射', '伤害分摊', '替身抵消', '复苏'],
  回复附加: ['魂力恢复', '精神恢复', '净化', '解控', '驱散增益', '护卫', '复苏', '治疗反转', '资源反灌'],
  情报附加: ['标记弱点', '共享视野', '目标锁定', '感知干扰', '认知扭曲', '破隐'],
  特殊附加: ['窃取增益', '隐身', '嘲讽', '护卫', '状态转移', '窃取护盾', '资源夺取', '资源反灌', '复活封锁'],
  代价附加: ['自损', '异常耗蓝', '随机副作用'],
};
