...
const 技能根层执行元数据键表_V1 = Object.freeze(['技能来源', '技能类型', '目标模型', '结算策略', '消耗', 'cast_time']);
...
function 收口执行效果条目_V1(value = {}, fallbackTargetModel = '敌方单体', recordViolation = () => {}) {
  ...
  const 参数 = {};
  const 参数原始 = value.参数 && typeof value.参数 === 'object' && !Array.isArray(value.参数) ? value.参数 : {};
  Object.entries(中文化技能机制参数键_V1(参数原始)).forEach(([key, raw]) => {
  ...
  Object.entries(value).forEach(([key, raw]) => {
    ...
    参数[可见键名] = 中文化技能机制参数键_V1(cloneJsonValue(raw));
  });
...
function 收口执行系统基础条目_V1(value = {}, fallbackTargetModel = '敌方单体', recordViolation = () => {}) {
  ...
  const 目标模型 = 归一化执行效果目标模型_V1(source.目标模型 || source.目标 || source.对象, fallbackTargetModel);
  const normalized = {
    机制: '系统基础',
    目标模型,
    对象: mapSkillTargetModelToCombatTarget(目标模型),
    技能类型: String(source.技能类型 || '').trim() || '无',
    结算策略: getSkillTargetResolutionStrategy(目标模型),
...
function wrapGrantableRuntimeEffectsForSupport(packedEffects = [], type = '强攻系', skillName = '') {
  ...
  packedEffects.push({
    机制: '机制授予',
    目标: type === '食物系' ? '食用者' : '友方单体',
    对象: type === '食物系' ? '食用者' : '友方单体',
...
function buildPackedGravityRatioEffect(target, gravityConfig = 2, duration = 0, trigger = '状态持续') {
