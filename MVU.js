...
const 技能根层执行元数据键表_V1 = Object.freeze(['技能来源', '技能类型', '目标模型', '结算策略', '消耗', 'cast_time', '前摇']);
...
function 规范化执行效果触发_V1(value = '') {
  ...
  return ['立即', '每回合', '命中后', '施放后', '回合结束', '状态结束', '常驻'].includes(normalized) ? normalized : normalized;
}
...
function 收口执行效果条目_V1(value = {}, fallbackTargetModel = '敌方单体', recordViolation = () => {}) {
  ...
  const 目标 = 由目标模型提取执行效果目标_V1(value.目标 || value.对象 || value.目标模型 || fallbackTargetModel, 由目标模型提取执行效果目标_V1(fallbackTargetModel, '单体'));
  const 持续回合 = Math.max(0, Math.round(Number(value.持续回合 ?? value.持续 ?? 0)));
  const 持续tick = Math.max(0, Math.round(Number(value.持续tick ?? 0)));
  const 有效期tick = Math.max(0, Math.round(Number(value.有效期tick ?? 0)));
  const 触发 = 规范化执行效果触发_V1(value.触发时机 || value.触发 || '');
  const normalized = { 机制, 目标 };
  const 参数原始 = value.参数 && typeof value.参数 === 'object' && !Array.isArray(value.参数) ? value.参数 : {};
  Object.entries(中文化技能机制参数值_V1(参数原始)).forEach(([key, raw]) => {
    if (!key || 技能执行黑名单键集合_V1.has(key)) return;
    if (raw === undefined) return;
    if (typeof raw === 'string' && !raw.trim()) return;
    normalized[key] = cloneJsonValue(raw);
  });
  Object.entries(value).forEach(([key, raw]) => {
    if (
      [
    ...
    if (可见键名 === '动作' || 可见键名 === '结算策略') return;
    normalized[可见键名] = 中文化技能机制参数值_V1(cloneJsonValue(raw));
  });
  if (normalized.属性 !== undefined) normalized.属性 = 中文化技能机制参数值_V1(normalized.属性);
  if (normalized.判定属性 !== undefined) normalized.判定属性 = 中文化技能机制参数值_V1(normalized.判定属性);
  if (normalized.数值 !== undefined) normalized.数值 = 规范化执行效果数值_V1(normalized.数值, value.动作);
  if (normalized.数值倍率 !== undefined && normalized.数值 === undefined) {
    normalized.数值 = 规范化执行效果数值_V1(normalized.数值倍率, value.动作);
    delete normalized.数值倍率;
  }
  ['使用效果', '授予效果'].forEach(key => {
    if (!Array.isArray(normalized[key])) return;
    normalized[key] = normalized[key]
      .map(effect => 收口执行效果条目_V1(effect, 目标, recordViolation))
      .map(effect => {
...
function 收口执行系统基础条目_V1(value = {}, fallbackTargetModel = '敌方单体', recordViolation = () => {}) {
  ...
  const normalized = {
    机制: '系统基础',
    技能类型: String(source.技能类型 || '').trim() || '无',
    消耗: 解析技能消耗结构_V1(source.消耗),
    前摇: Number(source.前摇 ?? source.cast_time ?? 0) || 0,
  };
  if (source.级别 !== undefined) normalized.级别 = cloneJsonValue(source.级别);
...
function 收口技能执行效果数组_V1(effectArray = [], options = {}) {
  ...
  const 系统基础目标模型 = 归一化执行效果目标模型_V1(systemSource.目标模型 || systemSource.对象, fallbackTargetModel);
  ...
  const effectList = source
    ...
    .map(effect => 收口执行效果条目_V1(effect, 系统基础目标模型, recordViolation))
    .filter(Boolean);
...
function buildPackedAttributeEffect(mechanism, target, property, action, value, duration = 0, trigger = '立即') {
  ...
  const effect = {
    ...
    属性: buildSkillEffectPropertyLabel(property),
  ...
  10: {
    ...
    _效果数组: [
      ...
      { 机制: '直接伤害', 目标: '敌方单体', 威力倍率: 300, 伤害类型: '物理近战', 穿透修饰: 50 },
      {
        机制: '斩杀补伤',
        目标: '敌方单体',
        判定属性: buildSkillJudgePropertyLabel('def_ratio'),
        判定阈值: 0.5,
...
Object.values(GoldenDragonSkills).forEach(skill => {
  if (skill && Array.isArray(skill._效果数组)) 收口技能执行结构_V1(skill, { 目标模型: '敌方单体' });
});
...
function buildSeventhRingTrueBodySkill(
  ...
  const 系统基础 = 效果数组.find(effect => String(effect?.机制 || '').trim() === '系统基础');
  if (系统基础) {
    系统基础.消耗 = 构建武魂真身消耗文本_V1(系别, grade, ringIndex, 释放形态, 食物工具分支);
    系统基础.对象 = '自身';
    系统基础.目标模型 = '自身';
    系统基础.技能类型 = 食物分支 ? '真身/食物增幅' : 食物工具分支 ? '真身/制作增幅' : '真身/增幅';
    系统基础.cast_time = 20;
    系统基础.级别 = quality;
  }
  if (食物分支) {
    const 造物 = 效果数组.find(effect => String(effect?.机制 || '').trim() === '造物生成');
    const 造物参数 = 造物?.参数 && typeof 造物.参数 === 'object' && !Array.isArray(造物.参数) ? 造物.参数 : 造物;
    if (造物参数 && Array.isArray(造物参数.使用效果)) {
      造物参数.可分发 = true;
      造物参数.触发方式 = '食用';
      造物参数.使用效果.forEach(effect => {
        if (!effect || typeof effect !== 'object') return;
        const 规则 = Array.isArray(effect.对象差异规则) ? effect.对象差异规则 : [];
        effect.对象差异规则 = [
          ...规则,
          { 条件: '自身', 处理: '倍率压制', 参数: { 数值倍率: 0.9 } },
          { 条件: '友方', 处理: '倍率提升', 参数: { 数值倍率: 1.05 } },
        ];
      });
    }
  }
...
function autoGenerateSkill(
  ...
  const 运行结算策略 = blueprint.释放形态 === '造物承载'
    ? getSkillTargetResolutionStrategy('自身')
    : (战斗.结算策略 || getSkillTargetResolutionStrategy(运行目标模型));
...
function ensureFusionSkillMentalCost(skill, currentRatio = 0.5) {
  ...
  const cost = 解析技能消耗结构_V1(systemBase.消耗);
