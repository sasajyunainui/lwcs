...
function 判断运行时角色间情报可见度_V1(观察者 = {}, 观察者名 = '', 目标 = {}, 目标名 = '') {
  ...
  const 生成可见度 = (状态, 依据, 写作口径, 限制 = '') => ({
    观察者: 观察名,
    目标: 目标名称,
    视角: `${观察名 || '未知观察者'} -> ${目标名称 || '未知目标'}`,
    状态,
    依据,
    限制,
    写作口径,
  });
  if (战斗记录 && Number(战斗记录.次数 || 0) > 0) return 生成可见度('战斗信息可见', `${观察名}曾与${目标名称}交手`, '可引用交手表现、战斗风格和已见过的能力', '不能知道交手中未暴露的底牌、私密档案或内心动机');
  ...
  if (目标声望 >= 5000) {
    ...
    return 声望圈层接近
      ? 生成可见度('公开详细可见', `${观察名}声望${观察者声望}，${目标名称}声望${目标声望}，声望差${声望差}`, '可引用公开身份、名望、公开事迹、传闻和常见战斗评价', '不能知道隐藏底牌、私密档案或未公开动机')
      : 生成可见度('公开传闻可见', `${目标名称}声望${目标声望}已成名，但${观察名}声望${观察者声望}，声望差${声望差}`, '只能引用姓名、公开身份、广泛流传名声和粗略传闻', `${观察名}不能直接获得${目标名称}的详细战斗档案、隐秘身份、弱点和私密动机`);
  }
  const 关系 = 目标名称 ? (观察者?.社交?.关系?.[目标名称] || {}) : {};
  const 关系名 = String(关系.关系 || '陌生');
  const 关系路线 = String(关系.关系路线 || '');
  const 好感度 = Number(关系.好感度 || 0);
  if (/敌对|死敌|宿敌|对手|仇敌/.test(`${关系名}${关系路线}`)) return 生成可见度('对手信息可见', `${观察名}对${目标名称}关系=${关系名}/${关系路线 || '敌对关系'}`, '可写针对性判断、戒备、弱点猜测和旧怨', '只限真实对手/旧怨视角；阵营泛敌对不能自动知道未暴露秘密');
  if (好感度 >= 30 && !/陌生|普通|路人/.test(关系名)) return 生成可见度('关系信息可见', `${观察名}对${目标名称}关系=${关系名}，好感度${好感度}`, '可引用关系内已知性格、习惯、身份和近期互动', '不能越过关系已知范围读取隐秘档案');
  return 生成可见度('详细情报受限', `${观察名}对${目标名称}关系=${关系名}，好感度${好感度}`, '只能写外观、现场表现和需要进一步确认的线索', `${观察名}不能直接获得${目标名称}的详细资料`);
}
...
function 构建运行时情报可见度索引_V1(数据根 = {}, 角色名集合 = new Set()) {
  ...
  const 写入边 = (观察者名, 目标名) => {
    ...
    const 可见度 = 判断运行时角色间情报可见度_V1(观察者, 观察者名, 目标, 目标名);
    if (!可见度) return;
    边列表.push(可见度);
  };
  const 相关角色名列表 = Array.from(角色名集合 || []).filter(角色名 => 数据根?.char?.[角色名]);
  相关角色名列表.forEach(角色名 => {
    写入边(玩家名, 角色名);
    写入边(角色名, 玩家名);
  });
  ...
  边列表.forEach(边 => {
    ...
    const 限制 = String(边?.限制 || '').trim();
    const 写作口径 = String(边?.写作口径 || '').trim();
    const 键 = JSON.stringify([观察者名, 状态, 限制, 写作口径]);
    if (!合并表.has(键)) 合并表.set(键, { 观察者: 观察者名, 状态, 目标: [], 依据: [], 限制, 写作口径 });
    const 分组 = 合并表.get(键);
    if (!分组.目标.includes(目标名)) 分组.目标.push(目标名);
    const 依据 = String(边?.依据 || '').trim();
    if (依据) 分组.依据.push(`${目标名}：${依据}`);
  });
  return Array.from(合并表.values());
}
...
function 清理MVU更新技能输出字段_V1(技能 = {}) {
  ...
  const 需要补产物 = 运行时文本需要补全_V1(技能.产物描述) && !!buildSkillProductReferenceText(技能._效果数组 || []);
  const 需要补描述 = 需要补画面 || 需要补效果 || 需要补产物;
  删除运行时只读字段_V1(技能);
...
function 生成MVU正文视图_V1(数据输入 = null, userInput = '', plotText = '') {
  ...
  地点名集合.forEach(地点名 => {
    ...
    const 清理后 = 清理地图运行时薄片_V1(清理正文排除式运行时值_V1(地点));
...
function 生成MVU更新视图_V1(数据输入 = null, userInput = '', aiText = '', plotText = '') {
  ...
  动态地点名集合.forEach(地点名 => {
    if (数据根?.world?.动态地点?.[地点名]) 视图.world.动态地点[地点名] = 清理地图运行时薄片_V1(cloneJsonValue(数据根.world.动态地点[地点名], {}));
  });
...
function 注入运行时技能默认提示_V1(skill = {}, context = {}) {
  ...
  if (hasPackedEffects && (!String(skill.产物描述 ?? '').trim() || String(skill.产物描述 ?? '').trim() === '无')) {
...
function 取角色主武魂系别_V1(char = {}, fallback = '强攻系') {
  ...
  const 第一武魂 = 条目.find(([键]) => 键 === '第一武魂')?.[1] || 条目[0]?.[1] || {};
  const 系别 = String(第一武魂?.系别 || '').trim();
...
const SKILL_PROTOTYPE_FIELD_OPTIONS_V1 = Object.freeze({
  ...
  状态: Object.freeze(['中毒', '流血', '灼烧', '冻伤', '持续创伤', '持续恢复', '迟缓', '资源燃烧', '眩晕', '沉默', '缴械', '致盲', '禁疗', '治疗反转', '隐匿', '护盾', '无敌', '霸体', '标记', '封技', '位移限制', '真身', '任意负面', '任意增益', '任意状态']),
...
function 条件分支替换效果等价_V1(parentEffect = {}, replacementEffects = []) {
  const 替换列表 = Array.isArray(replacementEffects) ? replacementEffects : [];
  if (替换列表.length !== 1) return false;
  const 基础 = 规范化等价比较效果条目_V1(parentEffect);
...
function 收口执行条件分支条目_V1(value = {}, recordViolation = () => {}, parentEffect = {}) {
  ...
  if (处理 === '替换效果' || 处理 === '追加效果') {
    ...
    if (处理 === '替换效果' && 条件分支替换效果等价_V1(parentEffect, 效果列表)) return null;
    normalized[字段名] = 效果列表;
  }
  return normalized;
}
...
function 构建缺少状态禁用条件分支_V1(状态 = '持续创伤') {
  return [{
...
function 补足机制模板原型数值_V1(原型 = '', 字段 = {}, source = {}) {
  ...
  if (原型 === '结算修正' && String(字段.结算 || source.结算 || '').trim() === '持续伤害引爆' && 字段.条件分支 === undefined) {
    字段.条件分支 = 构建缺少状态禁用条件分支_V1(String(source.状态 || source.状态名称 || '持续创伤').trim() || '持续创伤');
  }
  if (原型 === '状态时窗修正' && 字段.条件分支 === undefined) {
    字段.条件分支 = 构建缺少状态禁用条件分支_V1(String(字段.状态 || source.状态 || source.状态名称 || '持续创伤').trim() || '持续创伤');
  }
  ...
  if (原型 === '判定修正') {
    ...
    const 结算 = String(字段.结算 || '').trim();
      ...
      if (raw !== undefined) {
        ...
        if (结算 === '持续伤害引爆' && 字段.条件分支 === undefined) 字段.条件分支 = 构建缺少状态禁用条件分支_V1(String(source.状态 || source.状态名称 || '持续创伤').trim() || '持续创伤');
        return;
      }
  } else if (原型 === '状态时窗修正') {
    ...
    if (字段.条件分支 === undefined) 字段.条件分支 = 构建缺少状态禁用条件分支_V1(字段.状态 || '持续创伤');
    return;
  } else if (原型 === '目标选择修正') {
...
function 收口原型效果条目_V1(value = {}, fallbackTargetModel = '单体', recordViolation = () => {}, forcedPrototype = '', 上下文 = {}) {
  ...
  const 条件分支源 = Array.isArray(原型字段.条件分支) && 原型字段.条件分支.length
    ? [...(Array.isArray(value.条件分支) ? value.条件分支 : []), ...原型字段.条件分支]
    : (value.条件分支 || []);
  delete 原型字段.条件分支;
...
function buildPackedRecoverAttributeEffect(target, property, ratio, duration = 0, overtime = false) {
  ...
  if (!overtime) {
    return buildPackedAttributeEffect('属性变化', target, property, '加值', num, 0, '立即');
  }
  const 内部效果 = buildPackedAttributeEffect('属性变化', '自身', property, '加值', num, 0, '每回合');
  if (!内部效果) return null;
  return {
    原型: '状态施加',
    目标: 由目标模型提取执行效果目标_V1(target || '自身', '自身'),
...
function 转译技能结构目标_V1(值 = '') {
  return buildSkillTargetLabel(值 || '目标');
}
...
function 转译技能结构条件条目_V1(条件 = {}) {
  ...
  const 值 = 格式化技能结构转译字段_V1(条件?.值 ?? 条件?.状态 ?? 条件?.层级, '');
  const 对象文本 = 对象 === '目标' ? '目标' : 对象;
  if (类型 === '目标') {
    const map = { '==': '为', '!=': '不为' };
    return `${对象文本}${map[比较] || 比较}${值 || '指定目标'}`;
  }
  if (类型 === '状态') {
    const 状态 = 格式化技能结构转译字段_V1(条件?.状态 || 条件?.值, '指定状态');
    return `${对象文本}${比较 === '无' ? '没有' : '带有'}【${状态}】`;
  }
  if (类型 === '状态层级') {
    const 状态 = 格式化技能结构转译字段_V1(条件?.状态, '指定状态');
    const 层级 = 格式化技能结构转译字段_V1(条件?.层级 ?? 条件?.值, '1');
    return `${对象文本}【${状态}】层级${比较}${层级}`;
  }
  if (类型 === '护盾') return `${对象文本}${比较 === '无' ? '没有护盾' : '存在护盾'}`;
  if (['邪魂师', '深渊生物', '魂兽', '命中', '暴击', '被闪避'].includes(类型)) {
    return `${对象文本}${比较 === '无' ? '不满足' : '满足'}${类型}`;
  }
  if (/^(生命|体力|魂力|精神力)(比例|数值)$/.test(类型)) return `${对象文本}${类型}${比较}${值 || '0'}`;
  return `${对象文本}${类型}${比较}${值 || '指定值'}`;
}
...
function 转译技能结构条件分支_V1(分支 = {}, 深度 = 0) {
  ...
  if (处理 === '禁用基础效果') return `若${条件文本}，此效果不生效`;
  const 字段名 = 处理 === '追加效果' ? '追加效果' : '替换效果';
  if (处理 === '替换效果' && 条件分支替换效果等价_V1(分支?.__父效果 || {}, 分支?.[字段名] || [])) return '';
  const 效果文本 = 编译效果数组为人类语言_V1(分支?.[字段名] || [], { depth: 深度 + 1, maxDepth: 4 });
  if (处理 === '追加效果') return `若${条件文本}，追加：${效果文本 || '指定效果'}`;
  return `若${条件文本}，替换为：${效果文本 || '指定效果'}`;
}

function 转译技能结构附加语义_V1(效果 = {}, 深度 = 0) {
  const 片段 = [];
  const 生效方式 = String(效果?.生效方式 || '').trim();
  const 触发 = String(效果?.触发 || '').trim();
  ...
  const 层级 = 格式化技能结构转译字段_V1(效果?.层级, '');
  if (生效方式) 片段.push(`生效方式:${生效方式}`);
  if (触发) 片段.push(`触发:${触发}`);
  if (触发条件) 片段.push(`触发条件:${触发条件}`);
  if (持续回合 > 0) 片段.push(`持续${formatSkillNumber(持续回合)}回合`);
  if (持续tick > 0) 片段.push(`持续${formatSkillNumber(持续tick)}tick`);
  if (概率) 片段.push(`概率:${概率}`);
  if (层级) 片段.push(`层级:${层级}`);
  if (驱动属性 || 影响方向) 片段.push(`判定:${[驱动属性, 影响方向].filter(Boolean).join('/')}`);
...
function 转译单条执行效果_V1(效果 = {}, 选项 = {}) {
  ...
  if (!String(效果?.原型 || '').trim() && String(效果?.物品类型 || '').trim()) {
    const 物品类型 = String(效果.物品类型 || '魂技造物').trim();
    const 数量 = Math.max(1, Number(效果.数量 || 1));
    const 有效期 = Number(效果.有效期tick || 0) > 0 ? `，有效期${formatSkillNumber(效果.有效期tick)}tick` : '';
    const 使用词 = 物品类型 === '食物' ? '食用' : '使用';
    const 使用效果 = 编译效果数组为人类语言_V1(效果.使用效果 || [], { depth: 深度 + 1, maxDepth: 最大深度 });
    return `生成${物品类型}×${formatSkillNumber(数量)}${有效期}${使用效果 ? `；${使用词}后：${使用效果}` : ''}`;
  }
  const 原型 = String(效果?.原型 || '').trim();
  const 目标 = 转译技能结构目标_V1(效果?.目标 || 效果?.对象 || '目标');
  const 附加 = 转译技能结构附加语义_V1(效果, 深度);
  const 数值 = 格式化技能结构转译字段_V1(效果?.数值, '');
  const 嵌套 = (键, 标签) => {
    const 文本 = 编译效果数组为人类语言_V1(效果?.[键] || [], { depth: 深度 + 1, maxDepth: 最大深度 });
    return 文本 ? `；${标签}：${文本}` : '';
  };
  switch (原型) {
    case '伤害结算':
      return `对${目标}造成${格式化技能结构转译字段_V1(效果.伤害类型, '伤害')}，威力倍率${格式化技能结构转译字段_V1(效果.威力倍率, '0')}%${效果.攻击段数 ? `，${formatSkillNumber(效果.攻击段数)}段` : ''}${效果.防御穿透 ? `，防御穿透${格式化技能结构转译字段_V1(效果.防御穿透)}` : ''}${附加}`;
    case '延迟结算':
      return `延迟${formatSkillNumber(效果.延迟回合 || 1)}回合后结算${附加}${嵌套('结算效果', '结算效果')}`;
    case '资源变化':
      return `使${目标}${格式化技能结构转译字段_V1(效果.资源, '资源')}${数值 ? `变化${数值}` : '变化'}${附加}`;
    case '护盾变化':
      return `${/^-/.test(String(效果.数值 || '')) ? '削减' : '增加'}${目标}护盾${数值 || ''}${附加}`;
    case '属性修正':
      return `修正${目标}${格式化技能结构转译字段_V1(效果.属性, '属性')}${数值 || ''}${附加}`;
    case '判定修正':
      return `修正${目标}${格式化技能结构转译字段_V1(效果.判定, '判定')}${数值 || ''}${附加}`;
    case '行动打断':
      return `打断${目标}${效果.打断类型 ? `的${效果.打断类型}` : ''}${数值 ? `，强度${数值}` : ''}${附加}`;
    case '结算修正':
      return `修正${目标}${格式化技能结构转译字段_V1(效果.结算, '结算')}${数值 || ''}${附加}`;
    case '状态施加':
      return `对${目标}施加【${格式化技能结构转译字段_V1(效果.状态, '状态')}】${附加}${嵌套('状态效果', '状态内效果')}`;
    case '状态时窗修正':
      return `调整${目标}的【${格式化技能结构转译字段_V1(效果.状态, '状态')}】时窗：${格式化技能结构转译字段_V1(效果.调整方式, '调整')}${效果.调整回合 ? `${formatSkillNumber(效果.调整回合)}回合` : ''}${效果.结算倍率 ? `，结算${效果.结算倍率}` : ''}${附加}`;
    case '状态移除':
      return `移除${目标}${格式化技能结构转译字段_V1(效果.状态 || 效果.匹配原型 || 效果.资源 || '状态')}${效果.数量 ? `，数量${formatSkillNumber(效果.数量)}` : ''}${附加}`;
    case '规则防御':
      return `使${目标}获得规则防御【${格式化技能结构转译字段_V1(效果.规则, '规则')}】${效果.次数 ? `×${formatSkillNumber(效果.次数)}` : ''}${附加}`;
    case '状态转移':
      return `将${格式化技能结构转译字段_V1(效果.状态, '状态')}从${格式化技能结构转译字段_V1(效果.来源, '来源')}转移到${格式化技能结构转译字段_V1(效果.去向, '目标')}${效果.数量 ? `，数量${formatSkillNumber(效果.数量)}` : ''}${附加}`;
    case '状态交换':
      return `交换${目标}与自身的${格式化技能结构转译字段_V1(效果.状态 || 效果.范围, '状态')}${效果.数量 ? `，数量${formatSkillNumber(效果.数量)}` : ''}${附加}`;
    case '伤害链':
      return `对${目标}触发伤害链${数值 ? `，数值${数值}` : ''}${效果.数量 ? `，链数${formatSkillNumber(效果.数量)}` : ''}${附加}`;
    case '拆层转存':
      return `拆出${目标}的${格式化技能结构转译字段_V1(效果.状态, '状态')}并从${格式化技能结构转译字段_V1(效果.来源, '来源')}转存到${格式化技能结构转译字段_V1(效果.去向, '目标')}${附加}`;
    case '资源锁定':
      return `锁定${目标}${格式化技能结构转译字段_V1(效果.资源, '资源')}${数值 ? `，数值${数值}` : ''}${附加}`;
    case '规则改写':
      return `改写${目标}规则【${格式化技能结构转译字段_V1(效果.规则, '规则')}】${数值 ? `，数值${数值}` : ''}${附加}`;
    case '机制抹消':
      return `抹消${目标}机制【${格式化技能结构转译字段_V1(效果.抹消目标, '目标机制')}】${效果.数量 ? `，数量${formatSkillNumber(效果.数量)}` : ''}${效果.抹消方式 ? `，${效果.抹消方式}` : ''}${附加}`;
    case '机制窃取':
      return `窃取${目标}机制【${格式化技能结构转译字段_V1(效果.窃取目标, '目标机制')}】${效果.数量 ? `，数量${formatSkillNumber(效果.数量)}` : ''}${效果.保留回合 ? `，保留${formatSkillNumber(效果.保留回合)}回合` : ''}${附加}`;
    case '机制授予':
      return `授予${目标}临时机制${效果.可用次数 ? `，可用${formatSkillNumber(效果.可用次数)}次` : ''}${附加}${嵌套('授予效果', '授予效果')}`;
    case '复制执行':
      return `复制执行${目标}${格式化技能结构转译字段_V1(效果.复制类型, '技能')}${效果.技能个数 ? `，数量${formatSkillNumber(效果.技能个数)}` : ''}${效果.削减比例 ? `，削减${格式化技能结构转译字段_V1(效果.削减比例)}` : ''}${附加}`;
    case '时光回溯':
      return `使${目标}回溯${格式化技能结构转译字段_V1(效果.回溯范围, '生命/资源/状态')}${效果.次数 ? `，次数${formatSkillNumber(效果.次数)}` : ''}${附加}`;
    case '位移执行':
      return `使${目标}${格式化技能结构转译字段_V1(效果.位移类型, '位移')}${效果.距离 ? `，距离${formatSkillNumber(效果.距离)}` : ''}${效果.方位 ? `，方位${效果.方位}` : ''}${附加}`;
    case '目标选择修正':
      return `修正${目标}目标选择【${格式化技能结构转译字段_V1(效果.选择, '选择')}】${数值 ? `，数值${数值}` : ''}${附加}`;
    case '行动判断修正':
      return `修正${目标}行动判断【${格式化技能结构转译字段_V1(效果.判断, '判断')}】${数值 ? `，数值${数值}` : ''}${附加}`;
    case '修炼速度修正':
      return `修正${目标}${格式化技能结构转译字段_V1(效果.修炼类型, '修炼')}速度${数值 || ''}${效果.有效期tick ? `，有效${formatSkillNumber(效果.有效期tick)}tick` : ''}${附加}`;
    case '成长收益修正':
      return `修正${目标}${格式化技能结构转译字段_V1(效果.成长项, '成长收益')}${效果.属性 ? `/${格式化技能结构转译字段_V1(效果.属性)}` : ''}${数值 || ''}${效果.有效期tick ? `，有效${formatSkillNumber(效果.有效期tick)}tick` : ''}${附加}`;
    case '召唤生成':
      return `召唤【${格式化技能结构转译字段_V1(效果.召唤物名称, '召唤物')}】×${formatSkillNumber(效果.数量 || 1)}${效果.行动模式 ? `，${效果.行动模式}` : ''}${效果.继承属性比例 ? `，继承${格式化技能结构转译字段_V1(效果.继承属性比例)}` : ''}${附加}`;
    case '场地施加':
      return `展开场地【${格式化技能结构转译字段_V1(效果.场地名称, '场地效果')}】${附加}${嵌套('场地效果', '场地效果')}`;
    default: {
      const 注册原型 = SKILL_PROTOTYPE_REGISTRY_V1[原型];
      const 字段文本 = Object.entries(效果)
        .filter(([键]) => !['原型', '目标', '生效方式', '条件分支'].includes(键))
        .map(([键, 原始值]) => `${键}:${格式化技能结构转译字段_V1(原始值, '')}`)
        .filter(Boolean)
        .slice(0, 8)
        .join('，');
      return `执行${原型 || '未命名效果'}${注册原型?.描述 ? `（${注册原型.描述}）` : ''}${目标 ? `，目标:${目标}` : ''}${字段文本 ? `，${字段文本}` : ''}${附加}`;
    }
  }
}
...
function 编译效果数组为人类语言_V1(效果数组 = [], 选项 = {}) {
  const 深度 = Math.max(0, Number(选项.depth || 0));
  const 最大深度 = Math.max(1, Number(选项.maxDepth || 5));
  return (Array.isArray(效果数组) ? 效果数组 : [])
    .map(效果 => 转译单条执行效果_V1(效果, { depth: 深度, maxDepth: 最大深度 }))
    .filter(Boolean)
  ...
  return (Array.isArray(副作用列表) ? 副作用列表 : [])
    .map(条目 => {
      ...
      const 片段 = [
        ...
        条目.触发时机 ? `触发:${条目.触发时机}` : '',
        条目.生效对象 ? `对象:${条目.生效对象}` : '',
        Number(条目.触发概率 ?? 1) < 1 ? `概率:${formatSkillPercent(Number(条目.触发概率 || 0))}` : '',
        Number(条目.持续回合 || 0) > 0 ? `持续${formatSkillNumber(条目.持续回合)}回合` : '',
        条目.参数 ? `参数:${格式化技能结构转译字段_V1(条目.参数)}` : '',
      ];
      return 拼接技能结构转译片段_V1(片段);
    })
    .filter(Boolean)
    .join('；');
}
...
function 编译技能结构为人类语言_V1(技能或效果数组 = {}, 选项 = {}) {
  ...
  const 类型 = 格式化技能结构转译字段_V1(技能.技能类型 || 技能.type, '');
  const 消耗 = 格式化技能结构转译字段_V1(技能.消耗, '');
  const 前摇 = 技能.前摇 !== undefined ? Math.max(0, Number(技能.前摇 || 0)) : null;
  const 掌控度 = 技能.技能掌控度 && typeof 技能.技能掌控度 === 'object' ? 格式化技能结构转译字段_V1(技能.技能掌控度, '') : '';
  const 附带属性 = Array.isArray(技能.附带属性) && 技能.附带属性.length ? 技能.附带属性.join('/') : '';
  const 效果文本 = 编译效果数组为人类语言_V1(技能._效果数组 || [], { maxDepth: Math.max(1, Number(选项.maxDepth || 5)) });
  const 副作用文本 = 转译技能副作用列表_V1(技能.副作用列表 || 技能.sideEffects || []);
  if (名称) 片段.push(`技能:${名称}`);
  if (类型) 片段.push(`类型:${类型}`);
  if (消耗) 片段.push(`消耗:${消耗}`);
...
function buildSingleSkillEffectSummary(effect) {
  ...
  switch (原型) {
    ...
    case '机制授予': {
      const grantSummaries = (Array.isArray(effect?.授予效果) ? effect.授予效果 : [])
        .map(buildSingleSkillEffectSummary)
        .filter(Boolean);
      const grantText = grantSummaries.length
        ? grantSummaries.slice(0, 3).join('；')
        : String(effect?.授予名称 || '临时原型');
      return '使' + target + '获得一次性触发权：' + grantText + '，可用' + formatSkillNumber(effect?.可用次数 || 1) + '次';
    }
    case '复制执行':
...
function buildSkillProductReferenceText(packedEffects) {
  ...
  const visit = effect => {
    ...
    const itemType = String(effect.物品类型 || '').trim();
    if (itemType) {
      const ttl = Number(effect.有效期tick || 0);
      products.push(`${itemType}×${Math.max(1, Number(effect.数量 || 1))}${ttl > 0 ? `，有效${ttl}tick` : ''}`);
    }
    if (String(effect.原型 || '').trim() === '召唤生成') {
      const name = String(effect.召唤物名称 || '召唤物').trim() || '召唤物';
      const count = Math.max(1, Number(effect.数量 || 1));
      const turns = Number(effect.持续回合 || 0);
      const inherit = Number(effect.继承属性比例 || 0);
      products.push(`${name}×${count}${turns > 0 ? `，持续${turns}回合` : ''}${inherit > 0 ? `，继承约${Math.round(inherit * 100)}%属性` : ''}`);
    }
    if (String(effect.原型 || '').trim() === '场地施加') {
      const name = String(effect.场地名称 || '场地效果').trim() || '场地效果';
      const turns = Number(effect.持续回合 || 0);
      products.push(`${name}${turns > 0 ? `，持续${turns}回合` : ''}`);
    }
    ['使用效果', '授予效果', '状态效果', '结算效果', '场地效果'].forEach(key => {
...
function buildSkillEffectTodoText(packedEffects, skill = null) {
  ...
  const parts = [];
  if (referenceText) parts.push(`机制摘要：${referenceText}`);
...
function buildSkillVisualDescriptionFromPackedEffects(packedEffects, context = {}) {
  ...
  const parts = [];
  if (referenceText) parts.push(`机制摘要：${referenceText}`);
  return `待补全（${parts.join('；')}；只写释放、食用、召唤或成形时的画面，不重复机制条文，不写产物详细档案）`;
}
...
function autoGenerateSkill(
  ...
  const 食物效果已有自服分支 = effect => Array.isArray(effect?.条件分支) && effect.条件分支.some(branch => {
    const action = String(branch?.处理 || '').trim();
    const conditions = Array.isArray(branch?.条件) ? branch.条件 : [];
    return ['禁用基础效果', '替换效果'].includes(action) && conditions.some(condition =>
      String(condition?.类型 || '').trim() === '目标' &&
      String(condition?.值 || '').trim() === '自身',
    );
  });
  const 食物恢复效果需要自服禁用 = effect => {
    const 原型 = String(effect?.原型 || '').trim();
    if (原型 !== '资源变化') return false;
    const 资源 = String(effect?.资源 || '').trim();
    return ['生命', '体力', '魂力', '精神力'].includes(资源) && 读取生成校验比例数值_V1(effect.数值) > 0 && !/^-/.test(String(effect.数值 || ''));
  };
  const 食物增幅效果需要自服降效 = effect => {
    const 原型 = String(effect?.原型 || '').trim();
    if (shouldWrapSkillEffectAsGrant(effect)) return false;
    if (食物恢复效果需要自服禁用(effect)) return false;
    if (['属性修正', '判定修正', '结算修正', '护盾变化', '修炼速度修正', '成长收益修正'].includes(原型)) {
      return 读取生成校验比例数值_V1(effect.数值) > 0 && !/^-/.test(String(effect.数值 || ''));
    }
    return false;
  };
  ...
  if (main === '特殊规则类' && archetype === '引爆持续伤害')
    packedEffects.push(requireTargetDotState({
      原型: '结算修正',
      目标: normalizedTarget,
      结算: '持续伤害引爆',
  ...
  if (main === '特殊规则类' && archetype === '斩盾')
    packedEffects.push(requireTargetShield({
      原型: '护盾变化',
      目标: normalizedTarget,
  ...
  if (main === '特殊规则类' && archetype === '窃取护盾')
    ...
    }).map(effect => effect.目标 === '自身' ? effect : requireTargetShield(effect)));
  if (secondary.includes('状态转移'))
    packedEffects.push({
      原型: '状态转移',
      目标: normalizedTarget,
  ...
  if (是否食物系) {
    packedEffects.forEach(effect => {
      ...
      const 原型 = String(effect.原型 || '').trim();
      if (!原型) return;
      if (食物效果已有自服分支(effect)) return;
      if (shouldWrapSkillEffectAsGrant(effect)) return;
      if (食物恢复效果需要自服禁用(effect)) {
        appendPackedEffectConditionBranches(effect, 食物恢复自服禁用分支);
        return;
      }
      if (食物增幅效果需要自服降效(effect)) appendPackedEffectConditionBranches(effect, 食物自服效能调整分支);
    });
  }
...
if (typeof globalThis !== 'undefined') {
  ...
  globalThis.__LWCS_TRANSLATE_SKILL_EFFECTS__ = 编译技能结构为人类语言_V1;
  globalThis.__LWCS_NORMALIZE_SKILL_EXECUTION__ = 收口技能执行结构_V1;
  globalThis.__LWCS_DEBUG_EFFECT_EQ__ = (基础效果, 替换效果) => ({
    基础: 规范化等价比较效果条目_V1(基础效果),
    替换: 规范化等价比较效果条目_V1(Array.isArray(替换效果) ? 替换效果[0] : 替换效果),
    等价: 条件分支替换效果等价_V1(基础效果, Array.isArray(替换效果) ? 替换效果 : [替换效果]),
  });
}
...
const SkillStructSchema = z
  .object({
    ...
    产物描述: z.string().prefault('无').describe('造物承载、召唤物或场地实体的外观与用途描述；无产物时为无'),
    消耗: z.any().prefault('无'),
