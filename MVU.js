...
function 清理MVU更新角色可维护字段_V1(角色 = {}) {
  ...
  Object.values(角色.魂骨 || {}).forEach(魂骨 => 删除运行时对象字段_V1(魂骨, ['name', 'age', '状态', '属性加成']));
  ...
  删除运行时对象字段_V1(角色.精神领域, ['战斗修饰', '维护消耗']);
  return 角色;
}
...
function 清理MVU更新技能输出字段_V1(技能 = {}) {
  ...
  删除运行时只读字段_V1(技能);
  if (效果数组 && (需要补画面 || 需要补效果)) 技能._效果数组 = 效果数组;
...
function 直接自动生成技能结构_V1(skill = {}, context = {}) {
  ...
  skill.技能来源 = String(context?.sourceCategory || context?.技能来源 || skill?.技能来源 || '魂技').trim() || '魂技';
  ...
  黄金龙枪: {
    ...
    装备技能: {
      黄金龙噬: {
        ...
        _效果数组: [
          { 原型: '伤害结算', 目标: '单体', 威力倍率: 360, 伤害类型: '物理近战', 防御穿透: 35 },
          { 原型: '资源变化', 目标: '自身', 资源: '体力', 数值: '12%', 触发: '命中后' },
        ],
...
function 保留本轮已存在技能效果数组_V1(新变量 = {}, 旧变量 = {}) {
  ...
  const 是技能结构 = 对象 => {
    ...
    return ['魂技名', '技能来源', '技能类型', '效果描述', '消耗', '_效果数组'].some(键 =>
