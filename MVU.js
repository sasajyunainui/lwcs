...
function buildSpecialRuleArchetypeWeightedTableByContext(grade, type = '强攻系') {
  ...
  if (type === '精神系') {
    ...
    table = rebalanceWeightedTableWithPreferredValue(table, '吞噬', { C: 14, B: 22, A: 32, S: 44 }[grade] || 22);
    table = rebalanceWeightedTableWithPreferredValue(table, '能力共享', { C: 10, B: 16, A: 24, S: 34 }[grade] || 16);
  ...
  if (type === '元素系') {
    ...
    table = rebalanceWeightedTableWithPreferredValue(table, '吞噬', { C: 12, B: 18, A: 26, S: 34 }[grade] || 18);
  }
  if (type === '辅助系' || type === '治疗系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '窃取护盾', { C: 10, B: 16, A: 24, S: 30 }[grade] || 16);
    table = rebalanceWeightedTableWithPreferredValue(table, '能力共享', { C: 14, B: 22, A: 32, S: 42 }[grade] || 22);
  }
  if (type === '食物系') {
    table = rebalanceWeightedTableWithPreferredValue(table, '能力共享', { C: 12, B: 18, A: 28, S: 36 }[grade] || 18);
  }
  return table;
}

function getSpecialRuleExpansionChance(grade = 'B', type = '强攻系') {
  const base = { C: 6, B: 10, A: 16, S: 22 }[String(grade || 'B').toUpperCase()] || 10;
  const bonus =
    {
      精神系: 10,
      控制系: 8,
...
function autoGenerateSkill(
  ...
  const grade = overrideGrade || gradeInfo.grade;
  ...
  const blueprint =
    options?.blueprintOverride && typeof options.blueprintOverride === 'object'
      ...
      : rollSkillBlueprint(type, grade, ringIndex, preferredSecondary, {
...
function buildMinimalSkillRuntimeSystemBaseEffect(source = {}) {
  ...
  const 副作用列表 = normalizeSkillSideEffectList(raw?.副作用列表 || []);
  return {
    机制: '系统基础',
