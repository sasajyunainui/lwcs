...
const SKILL_MAIN_MECHANIC_POOL_V1 = {
  ...
  特殊规则类: {
    ...
    children: ['召唤', '分身', '复制', '反制', '转化', '状态交换', '强制绑定/锁定', '条件触发', '规则改写', '机制窃取', '炸环', '时光回溯', '气运干涉', '厄运反噬'],
  },
};
...
const SKILL_ARCHETYPE_POOL_V1 = {
  ...
  特殊规则类: ['召唤', '分身', '复制', '反制', '转化', '状态交换', '状态转移', '强制绑定/锁定', '条件触发', '规则改写', '重力倍率调整', '引爆持续伤害', '斩盾', '窃取护盾', '吞噬', '能力共享', '机制抹消', '机制窃取', '炸环', '时光回溯', '气运干涉', '厄运反噬', '资源燃烧', '资源锁定'],
};
...
const SKILL_MECHANISM_META_V1 = (() => {
  ...
  register('规则改写', {
    ...
    设计台参数定义: [select('rewriteType', '改写类型', 'REWRITE_TYPE'), num('rewriteRatio', '改写强度', '0.35')],
  });
  register('威力增幅', {
    可主机制: true,
    可副机制: true,
...
const SKILL_RUNTIME_CONSUMER_TO_PROTOTYPES_V1 = Object.freeze({
  ...
  judge_effect: [原型编译条目('规则改写', { 规则: '条件触发' })],
  ...
  luck_interference: [原型编译条目('判定修正', { 判定: '气运' })],
  misfortune_backlash: [原型编译条目('判定修正', { 判定: '厄运反噬触发' }), 原型编译条目('结算修正', { 结算: '厄运反噬伤害' })],
  self_random_variance: [原型编译条目('行动判断修正', { 判断: '自身随机波动' })],
...
const SKILL_PROTOTYPE_FIELD_OPTIONS_V1 = Object.freeze({
  ...
  条件: Object.freeze(['无', '命中后', '施放后', '回合结束', '状态结束', '生命低于25%', '生命低于50%', '目标有负面', '目标有增益']),
  ...
  判定: Object.freeze(['命中', '闪避', '反应', '控制成功', '控制抵抗', '打断', '气运', '厄运反噬触发']),
  ...
  规则: Object.freeze(['格挡', '免死', '替身', '复活', '抵消', '治疗反转', '条件触发', '伤害转治疗', '治疗转伤害', '效果反转']),
  机制: Object.freeze([...Object.keys(SKILL_MECHANISM_META_V1 || {}), '控制机制', '回复机制', '护盾', '隐匿', '增益'].sort()),
  来源: Object.freeze(['自身', '目标', '双方', '食用者', '召唤物', '装备者']),
  去向: Object.freeze(['自身', '目标', '双方', '食用者', '召唤物', '装备者']),
  范围: Object.freeze(['单个', '全部', '随机', '最低层级', '最高层级']),
  改写为: Object.freeze(['无效', '伤害', '治疗', '护盾', '消耗', '前摇', '目标', '状态']),
  复制类型: Object.freeze(['技能', '属性', '状态', '外貌', '自身镜像']),
  复制来源: Object.freeze(['目标', '自身', '最近技能', '已记录样本']),
  复制字段: Object.freeze(['全部', '威力倍率', '伤害类型', '属性', '状态', '外貌']),
  回溯对象: Object.freeze(['自身', '目标', '双方', '全场']),
  回溯范围: Object.freeze(['生命', '资源', '状态', '位置', '全部']),
  选择: Object.freeze(['锁定', '随机目标偏转', '护卫', '嘲讽', '追击', '弱点']),
  判断: Object.freeze(['混乱', '误判', '判断干扰', '判断抵抗', '感知', '自身随机波动']),
  位移类型: Object.freeze(['拉近', '击退', '换位', '瞬移', '脱离']),
  方位: Object.freeze(['前方', '后方', '左侧', '右侧', '远离目标', '靠近目标', '指定位置']),
  修炼类型: Object.freeze(['修炼', '冥想', '功法']),
  成长项: Object.freeze(['训练', '学习', '属性成长', '经验收益']),
  字段: Object.freeze(['发色', '瞳色', '体型', '身高', '特殊特征']),
  伪装类型: Object.freeze(['身份', '气息', '外貌', '阵营识别']),
  产物类型: Object.freeze(['食物', '药剂', '投掷物', '载体', '一次性物品', '魂技造物']),
...
const 构建原型字段定义_V1 = 字段名 => Object.freeze({
  字段: 字段名,
  类型: SKILL_PROTOTYPE_FIELD_TYPE_V1[字段名] || '文本',
  选项: 字段名 === '原型'
    ? Object.freeze(SKILL_PROTOTYPE_DEFINITION_LIST_V1.map(([原型]) => 原型))
    : (SKILL_PROTOTYPE_FIELD_OPTIONS_V1[字段名] || Object.freeze([])),
  默认值: SKILL_PROTOTYPE_FIELD_DEFAULT_V1[字段名],
...
const SKILL_PROTOTYPE_DEFINITION_LIST_V1 = Object.freeze([
  ['伤害结算', '战斗结算', '敌对', ['威力倍率', '伤害类型', '命中次数', '穿透修饰', '触发', '持续回合'], ['威力倍率', '伤害类型'], '造成直接、持续、多段或延迟伤害'],
  ['资源变化', '战斗结算', '上下文', ['资源', '数值', '触发', '持续回合', '持续tick'], ['资源', '数值'], '恢复或扣除生命、体力、魂力、精神力等资源'],
  ['护盾变化', '战斗结算', '上下文', ['数值', '触发', '持续回合'], ['数值'], '获得、削减、斩除或窃取护盾'],
  ['属性修正', '战斗结算', '上下文', ['属性', '数值', '持续回合'], ['属性', '数值'], '修正力量、防御、敏捷、资源上限等属性'],
  ['判定修正', '战斗结算', '上下文', ['判定', '数值', '层级', '持续回合'], ['判定', '数值'], '修正命中、闪避、反应、控制、打断、气运等判定'],
  ['结算修正', '战斗结算', '上下文', ['结算', '数值', '持续回合'], ['结算', '数值'], '修正最终伤害、治疗、护盾、消耗、前摇等结果'],
  ['状态施加', '战斗结算', '上下文', ['状态', '层级', '数值', '持续回合', '状态效果'], ['状态'], '施加中毒、冻伤、沉默、缴械、隐匿、霸体、无敌等概括状态'],
  ['状态移除', '战斗结算', '上下文', ['状态', '匹配原型', '资源', '数值方向', '数量', '层级', '概率'], [], '按状态名或状态内部原型效果移除状态'],
  ['规则防御', '战斗结算', '可赋予', ['规则', '次数', '层级', '触发'], ['规则', '次数'], '格挡、免死、替身、抵消、复活等防御规则'],
  ['状态转移', '战斗结算', '上下文', ['状态', '来源', '去向', '数量'], ['状态'], '把状态从一方转移到另一方'],
  ['状态交换', '战斗结算', '上下文', ['状态', '范围'], ['状态'], '交换双方状态或效果'],
  ['规则改写', '战斗结算', '上下文', ['规则', '数值', '改写为'], ['规则'], '改写治疗、护盾、伤害、目标、消耗等规则'],
  ['机制抹消', '战斗结算', '敌对', ['机制', '数量', '范围'], ['机制'], '封锁或移除目标机制'],
  ['机制窃取', '战斗结算', '敌对', ['机制', '数量', '保留回合'], ['机制'], '夺取增益、技能、护盾或状态'],
  ['机制授予', '战斗结算', '可赋予', ['授予效果', '可用次数', '触发'], ['授予效果'], '临时授予技能、状态或效果'],
  ['复制执行', '战斗结算', '上下文', ['复制类型', '复制来源', '复制字段', '保留回合'], ['复制类型'], '复制技能、属性、状态、外貌或自身镜像'],
  ['时光回溯', '战斗结算', '可赋予', ['回溯对象', '回溯范围', '回溯tick', '次数'], ['回溯对象'], '回退生命、资源、状态或位置'],
  ['位移执行', '行为推导', '上下文', ['位移类型', '距离', '方位'], ['位移类型'], '拉近、击退、换位、瞬移或脱离'],
  ['目标选择修正', '行为推导', '上下文', ['选择', '数值', '层级', '持续回合'], ['选择', '数值'], '影响随机目标、锁定、护卫、嘲讽和追击'],
  ['行动判断修正', '行为推导', '上下文', ['判断', '数值', '层级', '持续回合'], ['判断', '数值'], '影响混乱、误判、判断干扰和判断抵抗'],
  ['修炼速度修正', '战斗外', '可赋予', ['修炼类型', '数值', '有效期tick'], ['数值'], '修正修炼、冥想或功法运行速度'],
  ['成长收益修正', '战斗外', '可赋予', ['成长项', '属性', '数值', '有效期tick'], ['成长项', '数值'], '修正训练、学习、属性成长或经验收益'],
  ['外貌改写', '战斗外', '上下文', ['字段', '值', '有效期tick'], ['字段', '值'], '改写发色、瞳色、体型、身高或特殊特征'],
  ['伪装改写', '战斗外', '上下文', ['伪装类型', '值', '强度', '有效期tick'], ['伪装类型', '值'], '改写身份、气息、外貌或阵营识别'],
  ['召唤生成', '战斗结算', '召唤物', ['召唤物名称', '数量', '行动模式', '继承属性比例', '持续回合'], ['召唤物名称', '数量'], '生成战斗或非战斗召唤物'],
  ['造物生成', '战斗外', '上下文', ['产物类型', '数量', '有效期tick', '使用效果', '背包模板', '魂技名', '有效期间'], ['产物类型', '数量'], '生成食物、药剂、投掷物、载体或一次性物品'],
]);

const SKILL_PROTOTYPE_COMMON_FIELDS_V1 = Object.freeze(['原型', '目标', '条件', '对象差异规则']);
...
const SKILL_PROTOTYPE_REGISTRY_V1 = Object.freeze(
  Object.fromEntries(
    SKILL_PROTOTYPE_DEFINITION_LIST_V1.map(([原型, 类别, 默认方向, 允许字段, 必填字段, 描述]) => {
      ...
      const 字段定义表 = Object.fromEntries(字段列表.map(字段名 => [字段名, 构建原型字段定义_V1(字段名)]));
      return [
      原型,
      Object.freeze({
        原型,
...
function rollSkillBlueprint(type, grade, ringIndex, preferredSecondary = [], options = {}) {
  ...
  const delivery = type === '食物系' ? '造物承载' : ['分身', '召唤'].includes(subModel) ? '召唤承载' : pickRandom(deliveryPool) || '直接生效';
...
function normalizeBlueprintOverrideForAutoGenerate(blueprintOverride = {}, type = '强攻系', grade = 'B', ringIndex = 1, preferredSecondary = [], options = {}) {
  ...
  const delivery =
    ...
    (type === '食物系' ? '造物承载' : ['分身', '召唤'].includes(archetype) ? '召唤承载' : pickRandom(deliveryPool) || '直接生效');
  const attrHints =
...
const SKILL_EFFECT_TARGET_VALUES_V1 = Object.freeze(['自身', '单体', '群体', '全场', '食用者', '召唤物', '装备者']);
...
const 技能根层执行元数据键表_V1 = Object.freeze(['目标', '目标模型', '对象', '结算策略', 'cast_time']);
...
function 收口执行对象差异规则条目_V1(value = {}, recordViolation = () => {}, parentEffect = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  ...
  const 处理原始 = String(value.处理 || value.动作 || value.action || '改写数值').trim() || '改写数值';
  const 处理别名 = {
    覆盖: '改写数值',
    替换机制: '改写机制',
    倍率提升: '改写数值',
  ...
  const 处理 = 技能对象差异规则处理候选_V1.includes(处理候选) ? 处理候选 : '改写数值';
  ...
  Object.entries(value).forEach(([key, raw]) => {
    ...
    参数[技能机制可见键中文名映射_V1[key] || 技能机制属性键中文名映射_V1[key] || key] = 中文化技能机制参数值_V1(cloneJsonValue(raw));
  });
  if (参数.数值倍率 !== undefined && 参数.数值 === undefined) {
    const 基础数值 = parentEffect && parentEffect.数值 !== undefined
      ? parseSkillSignedChangeNumber(parentEffect.数值)
      : Number(parentEffect && parentEffect.原始数值倍率基准);
    const 倍率 = Number(参数.数值倍率);
    参数.数值 = Number.isFinite(基础数值) && Number.isFinite(倍率)
      ? formatSkillSignedChangeValue(基础数值 * 倍率, /%$/.test(String(parentEffect?.数值 ?? '')) || Math.abs(基础数值) <= 1)
      : 规范化执行效果数值_V1(参数.数值倍率, value.动作 || value.action || value.处理);
    delete 参数.数值倍率;
  }
  const normalized = {
    条件,
...
function 收口执行对象差异规则列表_V1(value = [], recordViolation = () => {}, parentEffect = {}) {
  return (Array.isArray(value) ? value : [])
    .map(item => 收口执行对象差异规则条目_V1(item, recordViolation, parentEffect))
    .filter(Boolean);
}
...
function 标准化原型字段值_V1(value) {
  ...
  Object.entries(value).forEach(([key, raw]) => {
    ...
    if (['目标模型', '对象', '结算策略', '动作', '触发方式', '状态持续', 'cast_time', '应用' + '原型', '参数', '表现语义', '推荐语义'].includes(可见键名)) return;
...
const 技能执行批量字段键表_V1 = Object.freeze(['原型', '属性', '资源', '判定', '结算', '状态', '类型']);
const 技能执行原型禁用字段集合_V1 = new Set(['目标模型', '对象', '结算策略', '动作', '触发方式', '状态持续', 'cast_time', '应用' + '原型', '参数']);
...
function 补足机制模板原型数值_V1(原型 = '', 字段 = {}, source = {}) {
  ...
  if (原型 === '伤害结算') {
    ...
    if (字段.命中次数 === undefined) {
      const rawHitCount = 取首个数值(['命中次数', '段数', 'hitCount', 'segmentCount']);
      if (rawHitCount !== undefined) 字段.命中次数 = Math.max(1, Math.round(Number(rawHitCount) || 1));
    }
    if (字段.穿透修饰 === undefined) {
      const rawPen = 取首个数值(['穿透修饰', '破甲比例', 'armor_pen']);
      if (rawPen !== undefined) 字段.穿透修饰 = Number(rawPen) || 0;
    }
    return;
  }
  if (原型 === '护盾变化') {
  ...
  if (原型 === '判定修正') {
    ...
    if (字段.状态 === undefined) 字段.状态 = String(source.状态 || source.状态类型 || '软控').trim() || '软控';
    if (字段.数量 === undefined) 字段.数量 = Math.max(1, Math.round(Number(source.转移数量 || source.数量 || 1)));
    const 转移类型 = String(source.转移类型 || '').trim();
    ...
    if (字段.产物类型 === undefined) 字段.产物类型 = String(source.产物类型 || source.类型 || '一次性物品').trim() || '一次性物品';
...
function 收口原型效果条目_V1(value = {}, fallbackTargetModel = '单体', recordViolation = () => {}, forcedPrototype = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  ...
  const 触发 = 规范化执行效果触发_V1(value.触发 || '');
  ...
  if (原型字段.判定属性 !== undefined) 原型字段.判定属性 = 中文化技能机制参数值_V1(原型字段.判定属性);
  ...
  ['使用效果', '授予效果', '状态效果'].forEach(key => {
    ...
    原型字段[key] = 嵌套源
      .flatMap(effect => 收口执行效果条目列表_V1(effect, 目标, recordViolation))
      .map(effect => {
  ...
  const 对象差异规则 = 收口执行对象差异规则列表_V1(value.对象差异规则 || [], recordViolation, normalized);
...
function 收口执行效果条目列表_V1(value = {}, fallbackTargetModel = '敌方单体', recordViolation = () => {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return [];
  const 批量条目列表 = 展开技能执行批量字段_V1(value);
  if (批量条目列表.length !== 1 || 批量条目列表[0] !== value) {
    return 批量条目列表.flatMap(entry => 收口执行效果条目列表_V1(entry, fallbackTargetModel, recordViolation));
  }
  const 显式原型 = String(value.原型 || '').trim();
  if (显式原型) {
    const normalized = 收口原型效果条目_V1(value, fallbackTargetModel, recordViolation, 显式原型);
    return normalized ? [normalized] : [];
  }
  const 机制 = String(value.机制 || '').trim();
  if (!机制 || 机制 === '系统基础' || isSkillSummaryEffect(value)) return [];
  const 原型列表 = 读取机制编译原型列表_V1(机制);
  if (!原型列表.length) return [];
  return 原型列表
    .map(原型条目 => 收口原型效果条目_V1(value, fallbackTargetModel, recordViolation, 原型条目))
    .filter(Boolean);
}
  ...
  if (directionTag === '锁定') {
    return [
      ...
      },
      {
        ...
        目标: targetModel,
        持续回合: duration,
        判定属性: buildSkillJudgePropertyLabel('agi'),
  ...
  if (directionTag === '转译') {
    return [
      ...
      },
      {
        机制: '条件触发',
        目标: targetModel,
        判定属性: buildSkillJudgePropertyLabel('hp_ratio'),
        判定阈值: 0.45,
...
function buildConstructUsableEffects(baseEffects, defaultTarget, skillType = '') {
  ...
  (Array.isArray(baseEffects) ? baseEffects : []).forEach(effect => {
    ...
    if (['直接伤害', '多段伤害', '持续伤害', '延迟爆发'].includes(mechanism) || effect.威力倍率) {
      const value = { 威力倍率: effect.威力倍率 || 0, 伤害类型: effect.伤害类型 || '无' };
      if (Number(effect.穿透修饰 || 0) > 0) value.穿透修饰 = effect.穿透修饰;
      usable.push({
        target,
        type: 'damage',
    ...
    if (
      ...
      const value = action === '加值' || action === '减值' || mechanism === '持续恢复'
        ? parseSkillSignedChangeNumber(effect.数值)
        : effect.数值;
...
function buildSkillCombatProfile(blueprint, qualityCtx = {}) {
  ...
  if (mutation.includes('自残换收益')) {
    额外体力消耗基准 = Math.max(5, Math.min(20, 5 + ringIndex));
  ...
  if (mutation.includes('技能复制'))
    state.特殊机制标识 = [state.特殊机制标识, '技能复制'].filter(v => v && v !== '无').join('/');
  if (mutation.includes('阵营错位/锁定'))
    state.特殊机制标识 = [state.特殊机制标识, '阵营错位/锁定'].filter(v => v && v !== '无').join('/');
  const 是持续释放 =
    archetype === '持续伤害' ||
    delivery === '范围展开' ||
    delivery === '召唤承载' ||
...
function buildCreationUsageEffects(packedEffects, type = '') {
  ...
  return clonePackedSkillEffects(getMeaningfulSkillEffects(packedEffects))
    .filter(effect => String(effect?.原型 || '') !== '造物生成')
    .map(effect => {
...
function shouldWrapSkillEffectAsGrant(effect = {}) {
  const mechanism = String(effect?.机制 || '').trim();
  if (!mechanism || ['系统基础', '机制授予', '造物生成', '生成造物'].includes(mechanism)) return false;
  if (
    [
      '属性变化',
      '持续恢复',
    ...
    return false;
  const meta = SKILL_MECHANISM_META_V1[mechanism] || {};
  return meta.目标语义 === '敌对' || meta.目标语义 === '上下文' || meta.目标语义 === '仅自身' || meta.仅自身 === true;
}
...
function wrapGrantableRuntimeEffectsForSupport(packedEffects = [], type = '强攻系', skillName = '') {
  ...
  const grantEffects = clonePackedSkillEffects(getMeaningfulSkillEffects(packedEffects)).filter(
    effect => !['造物生成', '生成造物'].includes(String(effect?.机制 || '')),
  );
  ...
  packedEffects.push({
    机制: '机制授予',
    目标: type === '食物系' ? '食用者' : '单体',
    授予名称: String(skillName || '临时机制'),
    触发条件: '下次有效行动',
    可用次数: 1,
    授予效果: grantEffects.filter(effect => String(effect?.机制 || '').trim() !== '机制授予'),
  });
  return true;
}
...
const SKILL_JUDGE_PROPERTY_LABELS = Object.freeze({
  str: '力量',
  def: '防御',
  agi: '敏捷',
  vit: '体力',
...
function buildSkillJudgePropertyLabel(property = '') {
  const key = String(property || '').trim();
  return SKILL_JUDGE_PROPERTY_LABELS[key] || buildSkillEffectPropertyLabel(key);
}
...
function buildPackedGravityRatioEffect(target, gravityConfig = 2, duration = 0, trigger = '') {
  ...
  if (持续回合 > 0) effect.持续回合 = 持续回合;
  if (触发) effect.触发 = 触发;
  if (Number.isFinite(ratio) && ratio > 0) effect.重力倍率 = ratio;
  if (Number.isFinite(minRatio) && minRatio > 0) effect.重力倍率下限 = minRatio;
  if (Number.isFinite(maxRatio) && maxRatio > 0) effect.重力倍率上限 = maxRatio;
  return effect;
}
  ...
  switch (机甲) {
    case '机制授予': {
      ...
      const grantedNames = (Array.isArray(effect?.授予效果) ? effect.授予效果 : [])
        .map(item => String(item?.机制 || '').trim())
        .filter(Boolean)
        .filter(name => name !== '系统基础');
      const grantText = grantedNames.length ? grantedNames.slice(0, 3).join('/') : String(effect?.授予名称 || '临时机制');
      return '使' + target + '获得[' + grantText + ']触发权，可用' + formatSkillNumber(useCount) + '次';
    }
    case '直接伤害': {
    ...
    case '条件触发':
      return '满足[' + String(effect.触发条件 || effect.判定属性 || '条件') + ']时触发预设效果';
    case '规则改写':
      return '改写' + target + '的结算规则，强度' + formatSkillPercent(effect.改写强度 || effect.rule_rewrite_ratio || 0);
    case '威力增幅':
    ...
    case '穿透':
      return '攻击穿透' + target + '的防御，穿透比例' + formatSkillPercent(effect.穿透比例 || effect.穿透修饰 || 0);
    case '吸血':
...
const GoldenDragonSkills = {
  1: {
    ...
    前摇: 10,
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 威力倍率: 200, 伤害类型: '物理近战', 穿透修饰: 40 },
    ],
  },
  2: {
    技能名称: '【第二层·金龙体】',
  ...
  3: {
    ...
    前摇: 15,
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 威力倍率: 150, 伤害类型: '物理远程', 穿透修饰: 10 },
  ...
  5: {
    ...
    前摇: 10,
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 威力倍率: 200, 伤害类型: '物理近战', 穿透修饰: 20 },
  ...
  10: {
    ...
    _效果数组: [
      ...
      { 原型: '伤害结算', 目标: '单体', 威力倍率: 300, 伤害类型: '物理近战', 穿透修饰: 50 },
  ...
  12: {
    ...
    前摇: 30,
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 威力倍率: 400, 伤害类型: '物理远程', 穿透修饰: 30 },
      { 原型: '状态施加', 目标: '群体', 状态: '位移限制', 持续回合: 1 },
      { 原型: '判定修正', 目标: '群体', 判定: '闪避', 数值: '-100%', 持续回合: 1 },
    ],
  },
  13: null,
  14: {
...
function 放大武魂真身效果倍率_V1(effect = {}, grade = 'B', 工具分支 = false) {
  if (!effect || typeof effect !== 'object') return effect;
  const 机制 = String(effect.机制 || '').trim();
...
function buildSeventhRingTrueBodySkill(
  ...
  skill.前摇 = 20;
  if (食物分支) {
    const 造物 = 效果数组.find(effect => String(effect?.机制 || '').trim() === '造物生成');
    if (造物 && Array.isArray(造物.使用效果)) {
      造物.可分发 = true;
      造物.使用效果.forEach(effect => {
        if (!effect || typeof effect !== 'object') return;
        const 规则 = Array.isArray(effect.对象差异规则) ? effect.对象差异规则 : [];
        ...
        effect.对象差异规则 = [
          ...规则,
          { 条件: '自身食用', 处理: '改写数值', 参数: { 数值: 计算改写数值(0.9) } },
          { 条件: '他人食用', 处理: '改写数值', 参数: { 数值: 计算改写数值(1.05) } },
        ];
      });
    }
  }
...
function autoGenerateSkill(
  ...
  const 食物自服效能调整规则 = Object.freeze([
    Object.freeze({ 条件: '自身', 处理: '改写数值', 参数: Object.freeze({ 自服系数: 0.72 }) }),
  ]);
  ...
  const appendPackedEffectObjectDiffRules = (effect, ruleList = []) => {
    ...
    const mergedRules = [...baseRules, ...clonePackedSkillEffects(ruleList).map(rule => {
      const next = cloneJsonValue(rule);
      const 系数 = Number(next?.参数?.自服系数);
      if (Number.isFinite(系数)) {
        const 基准数值 = effect.数值 !== undefined
          ? parseSkillSignedChangeNumber(规范化执行效果数值_V1(effect.数值, effect.动作))
          : Number(effect.数值);
        if (Number.isFinite(基准数值)) {
          next.参数 = { ...next.参数, 数值: formatSkillSignedChangeValue(基准数值 * 系数, Math.abs(基准数值) <= 1) };
        }
        delete next.参数.自服系数;
      }
      return next;
  ...
  if ((clash.基础威力倍率 || 0) > 0) {
    ...
    if (Number(clash.穿透修饰 || 0) > 0) damageEffect.穿透修饰 = clash.穿透修饰;
    const 吸血比例 = Number(((clash.瞬间恢复比例 || 0) / 100).toFixed(2));
    if (吸血比例 > 0) damageEffect.吸血比例 = 吸血比例;
  ...
  if (main === '感知/认知类' && archetype === '标记锁定')
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 0,
      判定属性: buildSkillJudgePropertyLabel('men_max'),
  ...
  if (main === '削弱类' && archetype === '元素封禁') {
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 1,
      封禁强度: Number(stateCalc.element_seal_ratio || 0),
      限定元素: Array.isArray(blueprint?.限定元素) ? blueprint.限定元素 : [],
      计算层效果: {
        element_seal_ratio: Number(stateCalc.element_seal_ratio || 0),
        final_damage_mult: Number(stateCalc.final_damage_mult || 1),
        cost_ratio: Number(stateCalc.cost_ratio || 1),
        windup_ratio: Number(stateCalc.windup_ratio || 1),
      },
    });
  }
  if (main === '增益类' && archetype === '威力增幅') {
    packedEffects.push({
      ...
      持续回合: state.持续回合 || 1,
      威力倍率: Number(stateCalc.final_damage_mult || 1),
      限定元素: Array.isArray(blueprint?.限定元素) ? blueprint.限定元素 : [],
      计算层效果: { final_damage_mult: Number(stateCalc.final_damage_mult || 1) },
    });
  }
  if (main === '增益类' && archetype === '技能效果增幅') {
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 1,
      效果倍率: Number(stateCalc.skill_effect_mult || 1),
      计算层效果: {
        skill_effect_mult: Number(stateCalc.skill_effect_mult || 1),
      },
    });
  }
  ...
  if (secondary.includes('无敌金身'))
    packedEffects.push({
      ...
      免疫等级阈值: Math.max(100, Number(state.计算层效果?.invincible_tier_threshold || 100)),
      damage_reduction: Number(state.计算层效果?.damage_reduction || 0),
    });
  if (secondary.includes('伤害反射'))
    packedEffects.push({
  ...
  if (main === '特殊规则类' && archetype === '资源燃烧')
    packedEffects.push({
      ...
      目标: normalizedTarget,
      资源类型: resourceDrainType,
      燃烧比例: Number(({ C: 0.1, B: 0.14, A: 0.2, S: 0.28, 'S+': 0.36 }[grade] || 0.14).toFixed(2)),
      转化比例: Number(({ C: 0.35, B: 0.45, A: 0.55, S: 0.7, 'S+': 0.85 }[grade] || 0.45).toFixed(2)),
    });
  ...
  if (secondary.includes('资源燃烧'))
    packedEffects.push({
      ...
      目标: normalizedTarget,
      资源类型: resourceDrainType,
      燃烧比例: Number((({ C: 0.06, B: 0.09, A: 0.12, S: 0.16 }[grade] || 0.09) * secondaryEffectScale).toFixed(2)),
      转化比例: Number(({ C: 0.25, B: 0.35, A: 0.45, S: 0.6 }[grade] || 0.35).toFixed(2)),
    });
  ...
  if (main === '特殊规则类' && archetype === '规则改写') {
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 1,
      改写类型: grade === 'S+' ? '综合反转' : '治疗反转',
      改写强度: Number(stateCalc.rule_rewrite_ratio || 0.3),
      计算层效果: {
        rule_rewrite_ratio: Number(stateCalc.rule_rewrite_ratio || 0.3),
      },
    });
  }
  if (main === '特殊规则类' && archetype === '机制窃取') {
    packedEffects.push({
      ...
      窃取比例: Number(stateCalc.mechanism_steal_ratio || 0.35),
      持续回合: state.持续回合 || 1,
    });
  }
  if (main === '特殊规则类' && archetype === '炸环') {
    packedEffects.push({
      机制: '炸环',
      目标: '自身',
      恢复时长tick: Math.max(1440, Math.round(4320 * ({ S: 1, 'S+': 0.75 }[grade] || 1))),
      威力倍率: Number(({ S: 1.8, 'S+': 2.2 }[grade] || 1.8).toFixed(2)),
      增幅字段: ['威力倍率', 'final_damage_mult', 'final_heal_mult', 'shield_gain_mult'],
    });
  }
  if (main === '特殊规则类' && archetype === '时光回溯') {
    packedEffects.push({
      ...
      持续回合: state.持续回合 || 1,
      回溯次数: Number(stateCalc.time_rewind_count || 1),
      恢复比例: Number(stateCalc.time_rewind_restore_ratio || 0.8),
      计算层效果: {
        time_rewind_count: Number(stateCalc.time_rewind_count || 1),
        time_rewind_restore_ratio: Number(stateCalc.time_rewind_restore_ratio || 0.8),
      },
    });
  }
  if (main === '特殊规则类' && archetype === '气运干涉') {
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 1,
      气运修正: Number(stateCalc.luck_modifier || 0),
      计算层效果: { luck_modifier: Number(stateCalc.luck_modifier || 0) },
    });
  }
  if (main === '特殊规则类' && archetype === '厄运反噬') {
    packedEffects.push({
      ...
      目标: normalizedTarget,
      持续回合: state.持续回合 || 1,
      判定率: Number(stateCalc.misfortune_check_rate || 0),
      反噬系数: Number(stateCalc.misfortune_backlash_ratio || 0),
      计算层效果: {
        misfortune_check_rate: Number(stateCalc.misfortune_check_rate || 0),
        misfortune_backlash_ratio: Number(stateCalc.misfortune_backlash_ratio || 0),
      },
    });
  }
  ...
  if (mutation.includes('阵营错位/锁定'))
    packedEffects.push({ 机制: '随机目标', 目标: '随机目标', 偏移概率: 0.35 });
  ...
  const shouldPackGenericState = (() => {
    ...
    if (
      main === '特殊规则类' &&
      ['条件触发', '转化', '复制', '状态交换', '规则改写', '机制窃取', '炸环', '时光回溯', '气运干涉', '厄运反噬'].includes(archetype)
    )
      return false;
  ...
  if (blueprint.释放形态 === '造物承载') {
    ...
    packedEffects.push({
      机制: '造物生成',
      目标: type === '食物系' ? '食用者' : 由目标模型提取执行效果目标_V1(normalizedTarget, '单体'),
      魂技名: itemName,
      产物类型: itemType,
...
function findPackedEffectByMechanism(效果数组 = [], mechanism = '') {
  ...
  return effects.find(effect => String(effect?.机制 || '').trim() === normalizedMechanism || String(effect?.原型 || '').trim() === normalizedMechanism) || null;
}
...
const AI_TODO_SKILL_VISUAL_STAGE1 =
  '待补全（请结合武魂特性、魂环来源魂兽特性、当前剧情上下文与机制选择（主机制/副机制1/副机制2/变异机制）补全发动画面，不新增机制）';
...
function buildSkillNameTodoText(context = {}) {
  ...
  return `待补全（填写魂技名；必须围绕【${spiritName}】同源命名并体现该武魂能力特征，禁止无关命名；需与机制选择（主机制/副机制1/副机制2/变异机制）保持一致。若为造物承载类技能，此名称同时作为生成物名称。${foodHint}）`;
}
...
function syncConstructSkillMetadata(skill = {}) {
  ...
  const createEffects = rawEffects.filter(effect => String(effect?.原型 || '') === '造物生成');
  ...
  skill._效果数组 = normalizedCreateEffects;
  return skill;
}
...
function 构建变异机制候选_V1(context = {}) {
  const 品质等级 = SKILL_GRADE_ORDER_V1[获取技能机制品质等级_V1(context)] || 1;
  const 魂环位 = Math.max(1, Number(context?.ringIndex || context?.魂环位 || 1));
  const 基础候选 = ['自残换收益', '增益附副作用', '自身也受影响', '随机目标', '高波动随机值'];
  if (品质等级 >= (SKILL_GRADE_ORDER_V1.C || 3)) 基础候选.push('回复转伤害', '伤害转回复');
  if (品质等级 >= (SKILL_GRADE_ORDER_V1.B || 4) || 魂环位 >= 5) 基础候选.push('效果反转');
  if (品质等级 >= (SKILL_GRADE_ORDER_V1.A || 5) || 魂环位 >= 7) 基础候选.push('技能复制', '阵营错位/锁定');
  return 规范化机制枚举数组_V1(基础候选).filter(item => SKILL_MUTATION_MECHANIC_TYPES_V1.includes(item));
}
...
function 构建技能机制决策临时数据_V1(skill = {}, context = {}) {
  ...
  const 变异机制候选 = 构建变异机制候选_V1(上下文);
  const 变异机制待生成提示词 = 构建变异机制待生成提示词_V1(变异机制候选);
...
function 按候选方案回填技能效果数组_V1(skill = {}, context = {}) {
  ...
  const 变异机制命中 = 提取机制限选命中值_V1(机制选择.变异机制, 构建变异机制候选_V1(context), {
    allowMultiple: true,
  });
  ...
  if (副作用条目) {
    效果数组.push(...将副作用条目转原型效果_V1(副作用条目));
  }
...
const TANGMEN_SECRET_SKILL_TEMPLATES = {
  ...
  紫极魔瞳: {
    ...
    前摇: 0,
    _效果数组: [
      { 原型: '判定修正', 目标: '自身', 判定: ['命中', '反应'], 数值: '+12%', 持续回合: 999 },
      { 原型: '目标选择修正', 目标: '自身', 选择: '锁定', 数值: '+1', 层级: 1, 持续回合: 999 },
      { 原型: '判定修正', 目标: '自身', 判定: '控制成功', 数值: '+8%', 持续回合: 999 },
    ],
  },
  暗器百解: {
    ...
    前摇: 0,
    _效果数组: [
      { 原型: '判定修正', 目标: '自身', 判定: ['命中', '打断'], 数值: '+10%', 持续回合: 999 },
...
const SkillStructSchema = z
  .object({
    ...
    机制决策临时: (() => {
      ...
      const 变异机制枚举 = 枚举回退(构建变异机制候选_V1(), '无');
