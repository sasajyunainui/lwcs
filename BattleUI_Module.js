...
class BattleUIComponent {
  ...
  initEngine() {
    ...
    function isBattleRecoverEffect(effect, properties = []) {
      ...
      return String(effect?.机制 || '').trim() === '属性变化' && String(effect?.动作 || '').trim() === '加值' && ['vit', 'sp', 'men'].includes(property);
    }
    ...
    function inferMainTypeFromEffects(skill) {
      ...
      if (hasSkillMechanism(skill, ['共享视野']) || getSkillEffects(skill).some(effect => isBattleAttributeSupportEffect(effect)))
        return '增益类';
    ...
    function deriveBattleSummaryFromEffects(skill, baseSummary = {}) {
      ...
      if (!summary.回复性质 || summary.回复性质 === '无') {
        if (getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect, ['vit']))) summary.回复性质 = '体力恢复';
        else if (getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect, ['sp', 'men']))) summary.回复性质 = '资源回复';
      }
      ...
      if (!baseSummary?.协同性 || baseSummary.协同性 === defaultSummary.协同性 || baseSummary.协同性 === '无') {
        if (
          ...
          summary.协同性 = '高';
        else if (targetText.includes('己方') || getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect)))
          summary.协同性 = '中';
        else summary.协同性 = '低';
      }
      if (!baseSummary?.生效方式 || baseSummary.生效方式 === defaultSummary.生效方式 || baseSummary.生效方式 === '无') {
        if (hasSkillMechanism(skill, ['受击反击', '格挡', '反制', '条件触发'])) summary.生效方式 = '触发';
          ...
          duration > 1 ||
          hasSkillMechanism(skill, ['持续伤害', '护盾', '减伤', '霸体', '免死', '共享视野', '召唤与场地'])
        )
          summary.生效方式 = '持续';
      ...
      if (!baseSummary?.持续性 || baseSummary.持续性 === defaultSummary.持续性 || baseSummary.持续性 === '无') {
        ...
        else if (hasSkillMechanism(skill, ['召唤与场地', '共享视野', '护盾', '减伤', '格挡', '霸体', '免死']))
          summary.持续性 = '中';
        else summary.持续性 = '无';
      }
  ...
  function applyStateToCharacter(targetChar, stateModule, sourceName, forceBuff) {
   ...
   strategicBranches.push({     name: '点燃生命之火',
      ...
      if (bestSpirit && defender.men >= 500) {
        ...
        strategicBranches.push({
          ...
          build() {
            return makeNpcAction(
              '召唤魂灵',
              `[战术召唤] NPC消耗精神力，将【${bestSpirit.表象名称}】实体化召唤至战场！(对标等级: ${maxSpiritLv})`,
            );
          },
        });
      }
    ...
    function buildNpcSkillCandidateContext(
      ...
      function pickSkillWithWeight(skills) {
        const weighted = (skills || []).map(skill => {
          ...
          const hasResourceRecover = getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect, ['sp', 'men']));
          ...
          if (mainType === '伤害类' && summary.目标规模 === '单体' && summary.爆发级别 === '高' && enemyHpRatio < 0.35)
            weight += 50;
      ...
      const defSkills = validSkills.filter(skill => {
        ...
        return (
          skillType === '防御' ||
          mainType === '防御类' ||
          hasSkillMechanism(skill, ['护盾', '减伤', '格挡', '霸体', '免死']) ||
          skillType === '控制'
        );
      });
      ...
      const recoverSkillPick = pickSkillWithWeight(validSkills.filter(skill => getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect))));
      ...
      const reactiveDefenseSkillPick = pickSkillWithWeight(
        validSkills.filter(skill => hasSkillMechanism(skill, ['受击反击', '反制', '格挡', '护盾', '免死', '霸体'])),
      );
    ...
    function scoreAllyTargetForSkill(actorChar, ally, skill) {
      ...
      if (getSkillEffects(skill).some(effect => isBattleRecoverEffect(effect, ['vit'])) || hasSkillMechanism(skill, ['护盾', '免死']))
    ...
    function settleTeamRoundEnd(combatData, logs) {
      const allUnits = [...(combatData.participants.team_player || []), ...(combatData.participants.team_enemy || [])];
