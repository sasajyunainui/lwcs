...
class BattleUIComponent {
  ...
  initEngine() {
    ...
    function 计算炸环等效级差_V1(年限 = 0) {
      const 数值 = Math.max(100, Number(年限 || 0));
      return Math.max(0.01, Math.log(1 + 数值 / 10000));
    }
    ...
    function fallbackPushSkill(actions, skill, fallbackName, category, 魂环路径 = null) {
      ...
      const 技能来源 = String(category || '魂技').trim() || '魂技';
      actions.push({
    ...
    function resolveIntentTargetNameFromAction(action, combatData) {
      ...
      const targetModel = 推断战斗技能目标模型(skill);
      if (['自身', '友方单体', '友方群体'].includes(targetModel)) return playerName;
      if (targetModel === '全场') return playerName || enemyName;
    ...
    const BATTLE_SKILL_CARRIER_TARGETS = new Set(['食用者', '召唤物', '装备者']);
    ...
    function normalizeBattleEffectTargetScale(value = '', fallback = '单体') {
      const text = String(value || '').trim();
      if (BATTLE_SKILL_CARRIER_TARGETS.has(text)) return text;
    ...
    function 推断战斗机制默认方向(effect = {}) {
      ...
      const mechanism = String(effect?.运行机制 || 原型 || '').trim();
      if (['食用者', '召唤物', '装备者'].includes(String(effect?.目标 || '').trim())) return '载体';
      if (mechanism === '召唤生成' || 原型默认方向 === '召唤物') return '无目标';
      ...
      if (原型 === '结算修正') {
        ...
        if (['最终伤害', '受到伤害', '反伤', '吸血', '防御穿透', '反击', '持续伤害引爆', '厄运反噬伤害'].includes(结算)) return '敌方';
        if (['最终治疗', '护盾获得', '技能效果'].includes(结算)) return '友方';
        if (['消耗', '前摇'].includes(结算)) return 读取战斗数值正负(effect?.数值) > 0 ? '敌方' : '友方';
      }
      return '敌方';
    }

    function 推断战斗效果目标模型(effect = {}, fallbackTargetModel = '敌方单体') {
      const rawTarget = String(effect?.目标 ?? '').trim();
      const scale = normalizeBattleEffectTargetScale(rawTarget, deriveBattleSkillTargetScaleFromModel(fallbackTargetModel));
      if (scale === '自身') return '自身';
      if (scale === '全场') return '全场';
      if (scale === '食用者') return '友方单体';
      if (scale === '召唤物' || scale === '装备者') return '自身';
    ...
    const 战斗机制中文键英文名 = Object.freeze({
      ...
      锁定层级: 'lock_level',
      增加控制成功: 'control_success_bonus',
      降低控制成功: 'control_success_penalty',
      随机偏转率: 'random_target_rate',
      跳过回合: 'skip_turn',
      无法反应: 'cannot_react',
      每回合伤害: 'dot_damage',
      ...
      抵消次数: 'substitute_count',
      格挡次数: 'block_count',
      复苏次数: 'revive_count',
      复苏回血比例: 'revive_heal_ratio',
      禁疗比例: 'heal_block_ratio',
      反转比例: 'heal_inversion_ratio',
      资源封锁比例: 'resource_block_ratio',
      反击倍率: 'counter_attack_ratio',
      吸血比例: 'life_steal_ratio',
    ...
    function 写入原型战斗字段(计算层效果, 字段名, 数值, 模式 = '增量', 原始文本 = '') {
      ...
      if (模式 === '倍率') {
        const raw = Number(数值);
        计算层效果[字段名] = raw > 0 && raw < 0.5 && /^[+-]/.test(String(原始文本 || '')) ? 1 + raw : raw;
        return;
      }
      计算层效果[字段名] = Number(数值 || 0);
    }

    function 编译原型战斗效果(effect = {}) {
      const 原型 = String(effect?.原型 || '').trim();
      if (!原型) return null;
      ...
      if (原型 === '伤害结算') {
        ...
        const 判定字段表 = {
          ...
          控制成功: 正向 ? 'control_success_bonus' : 'control_success_penalty',
          控制抵抗: 'control_resist_bonus',
        };
        写字段(判定字段表[判定], '增量', Math.abs(数值));
      } else if (原型 === '行动打断') {
        hydrated.运行机制 = '行动打断';
        hydrated.中断概率 = Math.abs(数值);
        写字段('interrupt_bonus', '增量', Math.abs(数值));
      } else if (原型 === '结算修正') {
        const 结算 = String(effect?.结算 || '').trim();
        const 结算字段表 = {
          最终伤害: ['final_damage_mult', '倍率'],
          最终治疗: ['final_heal_mult', '倍率'],
          护盾获得: ['shield_gain_bonus', '增量'],
          消耗: ['cost_ratio', '倍率'],
          前摇: ['windup_ratio', '倍率'],
          技能效果: ['skill_effect_mult', '倍率'],
          受到伤害: ['damage_reduction', '增量'],
          反伤: ['damage_reflect_ratio', '增量'],
          吸血: ['life_steal_ratio', '增量'],
          分摊: ['damage_share_ratio', '增量'],
          防御穿透: ['armor_pen', '增量'],
          反击: ['counter_attack_ratio', '增量'],
          持续伤害引爆: ['dot_detonate_ratio', '增量'],
          技能掌控度: ['control_success_bonus', '增量'],
          重力倍率: ['gravity_ratio', '增量'],
          自损增幅: ['final_damage_mult', '倍率'],
          炸环增幅: ['final_damage_mult', '倍率'],
          厄运反噬伤害: ['misfortune_backlash_ratio', '增量'],
        };
        ...
        if (属性) {
          const 面板倍率 = 数值 > -1 && Math.abs(数值) <= 1 ? Number((1 + 数值).toFixed(4)) : 数值;
        ...
        const 状态字段表 = {
          ...
          隐匿: ['stealth_level', '增量'],
          位移限制: ['lock_level', '次数'],
          禁疗: ['heal_block_ratio', '增量'],
          治疗反转: ['heal_inversion_ratio', '增量'],
          标记: ['lock_level', '次数'],
          护盾: ['shield_gain_bonus', '增量'],
        };
        ...
        if (Array.isArray(effect?.状态效果)) {
          ...
          effect.状态效果
            .flatMap(状态内效果 => 展开战斗原型数组字段(状态内效果))
            .map(状态内效果 => 编译原型战斗效果(状态内效果))
            .filter(Boolean)
            .forEach(状态内效果 => {
              if (状态内效果.计算层效果) {
                hydrated.计算层效果 = mergeCombatEffectMaps(
                  hydrated.计算层效果 || createEmptyCombatEffectMap(),
                  状态内效果.计算层效果,
                );
              }
              if (状态内效果.面板修改比例) {
                hydrated.面板修改比例 = { ...(hydrated.面板修改比例 || {}), ...状态内效果.面板修改比例 };
              }
            });
          effect.状态效果.forEach(状态内效果 => {
            if (
              String(状态内效果?.原型 || '').trim() === '资源变化' &&
              String(状态内效果?.资源 || '').trim() === '生命' &&
              读取战斗数值正负(状态内效果?.数值) < 0
            ) {
              写字段('dot_damage', '增量', Math.abs(读取战斗数值正负(状态内效果?.数值)));
            }
          });
        }
      } else if (原型 === '规则防御') {
        ...
        const 选择 = String(effect?.选择 || '').trim();
        const 选择字段表 = {
          锁定: ['lock_level', '次数'],
          随机目标偏转: ['random_target_rate', '增量'],
          护卫: ['damage_reduction', '增量'],
          嘲讽: ['random_target_rate', '增量'],
          追击: ['hit_bonus', '增量'],
          弱点: ['final_damage_mult', '倍率'],
        };
        const [字段名, 模式] = 选择字段表[选择] || [];
        写字段(字段名, 模式 || '增量', 模式 === '次数' ? 读取原型字段数值(effect, 1) : 数值);
        if (选择 === '嘲讽') 写字段('lock_level', '次数', Math.max(1, Number(effect?.层级 || 1)));
      } else if (原型 === '资源变化') {
        ...
        写字段('rule_rewrite_ratio', '增量');
      } else if (原型 === '行动判断修正') {
        const 判断 = String(effect?.判断 || '').trim();
        const 判断字段表 = {
          混乱: ['random_target_rate', '增量'],
          误判: ['random_target_rate', '增量'],
    ...
    function hydrateBattleExecutionEffectEntry(effect = {}, fallbackTargetModel = '敌方单体') {
      ...
      const sourceEffect = effect?.原型 ? 编译原型战斗效果(effect) : null;
      if (!sourceEffect || typeof sourceEffect !== 'object' || Array.isArray(sourceEffect)) return null;
      const 运行机制 = String(sourceEffect?.运行机制 || sourceEffect?.原型 || '').trim();
      if (!运行机制 || 运行机制 === '系统基础') return null;
      const targetModel = 推断战斗效果目标模型(sourceEffect, fallbackTargetModel);
      const duration = Math.max(0, Math.round(Number(sourceEffect?.持续回合 ?? 0)));
      const trigger = String(sourceEffect?.触发 || '').trim();
      const hydrated = {
        ...deepClonePlain(sourceEffect || {}),
        目标模型: targetModel,
        对象: targetModel,
      };
      if (!String(hydrated.目标 || '').trim()) hydrated.目标 = String(effect?.目标 || targetModel || '').trim();
      delete hydrated.机制;
      delete hydrated.参数;
    ...
    function 原型驱动缩放(effect = {}, directPayload = {}, hydrated = {}, attacker = {}, attackerFinalStat = {}, defender = {}, defenderFinalStat = {}) {
      ...
      if (影响方向 === '成功率') {
        ['control_success_bonus', 'interrupt_bonus', 'rule_rewrite_ratio', 'mechanism_steal_ratio', 'random_target_rate'].forEach(key => {
        ...
        if (directPayload.control_success_bonus === undefined) directPayload.control_success_bonus = Math.max(0, Number((缩放系数 - 1).toFixed(4)));
        return;
      }
      if (影响方向 === '持续时间') {
      ...
      if (影响方向 === '层级压制') {
        if (hydrated.层级 !== undefined) hydrated.层级 = scaleBattleLockLevel(hydrated.层级, 缩放系数);
        ['lock_level', 'stealth_level'].forEach(key => {
          if (directPayload[key] !== undefined) directPayload[key] = scaleBattleLockLevel(directPayload[key], 缩放系数);
        });
        return;
      }
      if (影响方向 === '消耗') {
    ...
    const BATTLE_MECHANISM_CONSUMERS = Object.freeze({
      ...
      mechanism_suppress(ctx) {
        ...
        state.抹消目标 = normalizeBattleMechanismSuppressionTargets(
          ctx.effect?.抹消目标 || '复苏',
        );
        state.抹消方式 = normalizeBattleMechanismSuppressionMode(
          缩放 >= 0.65 ? ctx.effect?.抹消方式 || '移除并封锁' : '仅封锁后续',
        );
    ...
    function applyRuntimeMechanismEffects(skill, attacker, attackerFinalStat, defender, defenderFinalStat, pState, context = {}) {
      ...
      effects.forEach(effect => {
        ...
        const mechanism = 运行效果?.运行机制 || 原型名 || '';
        const targetModel = 推断战斗效果目标模型(运行效果, String(运行效果?.目标 || effect?.目标 || '敌方单体'));
        const 允许生成状态壳 = ['状态施加', '规则防御', '状态时窗修正', '目标选择修正', '行动判断修正', '结算修正', '属性修正'].includes(原型名);
        const ensureStateShell = (fallbackName, extraFlags = []) => {
          if (!允许生成状态壳) return;
          const nextName = String(运行效果?.状态名称 || fallbackName || mechanism || '无');
          ...
          if (mechanism === '消耗提高' || mechanism === '消耗降低') {
            const value = 读取战斗效果数值(effect?.数值, 1);
          ...
          if (mechanism === '掌控修正') {
            ...
            const action = String(effect?.动作 || '').trim();
            if (action === '倍率压制' || (!action && value < 1)) directPayload.control_success_penalty = Math.max(0, 1 - value);
          ...
          if (原型名) {
            ...
            if (原型名 === '状态施加') ensureStateShell(运行效果?.状态名称 || mechanism, [mechanism]);
          }
    ...
    function 动作免除经验前摇折扣(动作 = {}) {
      ...
      const 技能来源 = String(getBattleSkillSourceCategory(技能) || 技能?.source_tag || '').trim();
      return (
        /领域/.test(动作类型) ||
        动作类型 === '穿戴装备' ||
        /斗铠|机甲/.test(动作类型) ||
    ...
    function getBattleSkillSummaryEffects(skill = {}) {
      const effects = getSkillEffects(skill)
        ...
        .map(effect => {
          ...
          const runtimeMeta = getBattleSkillMechanismMeta(mechanism) || {};
          const targetModel = 推断战斗效果目标模型(effect, getSkillRuntimeMeta(skill)?.目标模型 || '敌方单体');
    ...
    function pickBattleSkillBranchForAi(skill = {}, context = {}) {
      ...
      分支列表.forEach(分支标记 => {
        ...
        const 分支机制集合 = new Set(分支效果列表.map(effect => 读取战斗效果标签(effect)).filter(Boolean));
    ...
    function 解析条件分支对象(对象 = '目标', context = {}) {
      ...
      if (key === '食用者') return context.eater || context.user || context.actor || context.caster || null;
      if (key === '命中目标') return context.hitTarget || context.target || null;
      if (key === '召唤物') return context.summon || context.target || null;
      if (key === '装备者') return context.equipper || context.actor || context.caster || null;
    ...
    function 条件分支命中(condition = {}, context = {}) {
      ...
      if (!unit && !['命中', '暴击', '被闪避'].includes(type)) return false;
      if (type === '存活') return 比较条件分支数值(isCombatUnitAlive(unit) ? '是' : '否', op, value || '是');
      if (['性别', '年龄', '等级', '系别', '身份', '物种', '受伤部位', '当前行动', '当前领域', '位置'].includes(type)) {
        const fieldMap = { 身份: '主身份', 物种: '种族', 当前行动: '行动', 当前领域: '当前领域' };
        return 比较条件分支数值(读取条件分支单位字段(unit, fieldMap[type] || type), op, value);
      }
      const speciesFlags = getCombatTargetSpeciesFlags(unit);
      if (type === '邪魂师') return op === '无' ? !isCombatUnitEvilSoulMaster(unit) : isCombatUnitEvilSoulMaster(unit);
      if (type === '深渊生物') return op === '无' ? !speciesFlags.isAbyss : speciesFlags.isAbyss;
      if (type === '魂兽') return op === '无' ? !speciesFlags.isBeast : speciesFlags.isBeast;
      ...
      if (type === '状态' || type === '状态层级') {
        ...
        const stateName = String(condition.状态 || value || '').trim();
        const entry = stateName
          ? (
              stateMap[stateName] ||
              Object.values(stateMap).find(state => {
                const safeState = state && typeof state === 'object' ? state : {};
                if (String(safeState.状态 || safeState.状态名称 || '').trim() === stateName) return true;
                const 状态效果列表 = Array.isArray(safeState.状态效果) ? safeState.状态效果 : [];
                if (['持续创伤', '中毒', '流血', '灼烧', '冻伤'].includes(stateName)) {
                  return 状态效果列表.some(item =>
                    String(item?.原型 || '').trim() === '资源变化' &&
                    String(item?.资源 || '').trim() === '生命' &&
                    读取战斗数值正负(item?.数值) < 0,
                  ) || Number(safeState?.战斗效果?.dot_damage || 0) > 0;
                }
                return false;
              })
            )
          : null;
        if (type === '状态') return op === '无' ? !entry : !!entry;
        const layer = Number(entry?.层级 ?? entry?.层数 ?? 0);
        return !!entry && 比较条件分支数值(layer, op === '有' ? '>=' : op, condition.层级 || value || 1);
      }
      if (type === '护盾') {
      ...
      if (type === '暴击') return op === '无' ? !context.critical : !!context.critical;
    ...
    function getSkillEffects(skill, context = {}) {
      const 运行时目标模型 = 推断战斗技能目标模型(skill);
      ...
      const shouldKeepByEffectiveMode = effect => {
        ...
        if (context?.行为规划 === true || context?.排除跟随主原型 === true) return false;
    ...
    function applyAttributeCoeffToCombatSkill(skill = {}) {
      ...
      skill.cast_time = Math.max(
        ...
        Math.round(Number(skill.cast_time || skill.前摇 || 0) * normalizedCoeff.前摇),
      );
      skill.前摇 = skill.cast_time;
    ...
    function 按技能掌控度缩放技能(skill = {}, 施术者 = {}) {
      ...
      const 缩放效果列表 = effects => (Array.isArray(effects) ? effects : []).forEach(effect => {
        if (!effect || typeof effect !== 'object' || String(effect.运行机制 || effect.原型 || '').trim() === '系统基础') return;
        ...
        ['使用效果', '授予效果', '状态效果'].forEach(key => 缩放效果列表(effect[key]));
    ...
    function getSkillRuntimeMeta(skill) {
      ...
      const baseResolutionStrategy = deriveBattleTargetResolutionStrategy(baseTargetModel);
      ...
      return {
        来源分类: String(skill?.source_tag || '魂技').trim() || '魂技',
        技能类型: normalizeSkillTypeLabel(skill?.技能类型 || baseType || '无'),
    ...
    function getBattleSkillSourceCategory(skill) {
      return String(getSkillRuntimeMeta(skill).来源分类 || '魂技').trim() || '魂技';
    }
    ...
    function hasSkillMechanism(skill, mechanismList = []) {
      const targets = new Set((Array.isArray(mechanismList) ? mechanismList : [mechanismList]).map(item => String(item || '').trim()).filter(Boolean));
      ...
      return Array.from(
        new Set(
          getSkillEffects(skill)
            .flatMap(effect => [effect?.原型, effect?.运行机制])
    ...
    function getActiveMechanismSuppressionEntries(targetChar) {
      ...
      return Object.entries(targetChar.状态效果)
        .map(([key, cond]) => ({
          key,
          cond,
          tags: normalizeBattleMechanismSuppressionTargets(cond?.抹消目标 || cond?.窃取目标 || []),
          mode: normalizeBattleMechanismSuppressionMode(cond?.抹消方式 || ''),
        }))
        ...
        new Set(
          getSkillEffects(skill)
            .flatMap(effect => {
              ...
              const labels = [effect?.原型]
                .map(label => String(label || '').trim())
                .filter(Boolean);
              return [direct, ...labels.map(label => String(getBattleSkillMechanismMeta(label)?.运行时消费器 || '').trim())];
            })
            .filter(Boolean),
        ),
      );
    ...
    function getBattleSkillMechanismSuppressionTargets(skill) {
      const effect =
        getSkillEffects(skill).find(item => item?.原型 === '机制抹消' || item?.运行机制 === '机制抹消') ||
        null;
    ...
    function isBattleRecoverEffect(effect, properties = []) {
      const resourcePropertyMap = { 生命: 'vit', 体力: 'vit', 魂力: 'sp', 精神力: 'men' };
      ...
      if (String(effect?.原型 || '').trim() === '资源变化') return 读取战斗数值正负(effect?.数值) > 0 && 属性列表.some(property => ['vit', 'sp', 'men'].includes(property));
      const mechanism = String(effect?.原型 || '').trim();
    ...
    function isBattleAttributeSupportEffect(effect) {
      return ['属性修正', '判定修正', '结算修正', '护盾变化', '规则防御', '状态施加', '资源变化'].includes(String(effect?.原型 || '').trim());
    }

    function isBattleDebuffAttributeEffect(effect) {
      const prototype = String(effect?.原型 || '').trim();
      if (['属性修正', '判定修正', '结算修正', '资源变化', '护盾变化'].includes(prototype)) return 读取战斗数值正负(effect?.数值) < 0;
      return false;
    }
    ...
    function inferSkillTypeFromEffects(skill) {
      const fromSystem = normalizeSkillTypeLabel(skill?.技能类型);
    ...
    function deriveBattleSummaryFromEffects(skill, baseSummary = {}) {
      ...
      const runtimeMeta = getSkillRuntimeMeta(skill);
      const 技能来源 = String(runtimeMeta.来源分类 || '魂技').trim() || '魂技';
    ...
    function buildConditionTacticalSnapshot(entity) {
      ...
      const hasDotPressure = entries.some(
        ([name, cond]) =>
          Number(cond?.战斗效果?.dot_damage || 0) > 0,
      );
    ...
    function normalizeSkillData(skill, fallbackName = '未知技能') {
      ...
      delete normalized.目标模型;
      delete normalized.对象;
      delete normalized.结算策略;
    ...
    function buildFusionCombatSkill(fusionSkill = {}, fusionName = '武魂融合技', charData = null) {
      ...
      const baseCastTime = Number(skill.前摇 ?? skill.cast_time ?? 0) || 0;
    ...
    function parseSkillCostForChar(skill, char) {
      ...
      if (String(skill?.source_tag || '').trim() === '武魂融合技' && skill?.__融合可用 === false) {
    ...
    function projectPassiveSkillToConditions(char, skill) {
      ...
      getSkillEffects(skill).forEach((effect, index) => {
        const mechanism = effect?.原型 || '';
        if (effect?.原型 === '状态施加' && effect?.状态名称 && effect.状态名称 !== '无') {
    ...
    function resolveSkillTargetContext(skill, attacker, defender, combatData, effect = null) {
      const runtimeMeta = getSkillRuntimeMeta(skill);
      const baseTargetModel = effect
        ? 推断战斗效果目标模型(effect, runtimeMeta.目标模型 || '敌方单体')
        : normalizeBattleSkillTargetModel(runtimeMeta.目标模型 || '敌方单体', '敌方单体');
      const baseTargetScale = normalizeBattleSkillTargetScale(
        runtimeMeta?.目标规模 || '',
        deriveBattleSkillTargetScaleFromModel(baseTargetModel),
      );
      ...
      const resolutionStrategy = deriveBattleTargetResolutionStrategy(baseTargetModel);
    ...
    function 读取状态移除数量(value = 1) {
      const text = String(value ?? '').trim();
      if (text === '全部') return Infinity;
      const count = Math.max(1, Math.round(Number(text || 1)));
      return Number.isFinite(count) ? count : 1;
    }

    function 读取状态移除概率(value = '') {
      const text = String(value ?? '').trim();
      if (!text) return 1;
      const raw = 读取战斗数值正负(text);
      if (/%$/.test(text)) return Math.max(0, Math.min(1, Math.abs(raw)));
      const numeric = Number(text);
      if (Number.isFinite(numeric)) return Math.max(0, Math.min(1, numeric > 1 ? numeric / 100 : numeric));
      return 1;
    }

    function 状态条目匹配移除原型(cond = {}, effect = {}) {
      const 匹配原型 = String(effect?.匹配原型 || '').trim();
      if (!匹配原型 || 匹配原型 === '无') return true;
      const 状态效果列表 = Array.isArray(cond?.状态效果) ? cond.状态效果 : [];
      const 资源 = String(effect?.资源 || '').trim();
      const 数值方向 = String(effect?.数值方向 || '任意').trim() || '任意';
      const matchesDirection = value => {
        const signed = 读取战斗数值正负(value);
        if (数值方向 === '负向') return signed < 0;
        if (数值方向 === '正向') return signed > 0;
        return true;
      };
      if (状态效果列表.some(item => {
        if (String(item?.原型 || '').trim() !== 匹配原型) return false;
        if (资源 && String(item?.资源 || '').trim() !== 资源) return false;
        if (item?.数值 !== undefined && !matchesDirection(item.数值)) return false;
        return true;
      })) return true;
      const ce = cond?.战斗效果 || {};
      if (匹配原型 === '资源变化' && 资源 === '生命' && 数值方向 === '负向') return Number(ce.dot_damage || 0) > 0;
      if (匹配原型 === '护盾变化') return Number(cond?.shield_value || 0) > 0 || Number(ce.shield_gain_bonus || 0) > 0;
      return false;
    }

    function removeConditionsByPrototypeFilter(targetChar, effect = {}) {
      if (!targetChar?.状态效果) return [];
      const 状态 = String(effect?.状态 || '').trim();
      const maxLayer = Math.max(0, Number(effect?.层级 || 0));
      const maxCount = 读取状态移除数量(effect?.数量 ?? 1);
      const chance = 读取状态移除概率(effect?.概率);
      const removed = [];
      const typeMatches = (key, cond = {}) => {
        if (!状态 || 状态 === '任意状态') return true;
        if (状态 === '任意负面') return String(cond?.类型 || '') === 'debuff';
        if (状态 === '任意增益') return String(cond?.类型 || '') === 'buff';
        return key === 状态 || String(cond?.状态 || '').trim() === 状态;
      };
      const candidates = Object.entries(targetChar.状态效果)
        .filter(([key]) => !String(key || '').startsWith(AUTO_PROJECTED_CONDITION_PREFIX))
        .filter(([key, cond]) => typeMatches(key, cond))
        .filter(([, cond]) => !maxLayer || Math.max(0, Number(cond?.层级 || cond?.层数 || 0)) <= maxLayer)
        .filter(([, cond]) => 状态条目匹配移除原型(cond, effect));
      for (const [key] of candidates) {
        if (removed.length >= maxCount) break;
    ...
    function settleConditionsAtRoundEnd(char, label) {
        ...
        function updateActorFocusFromAction(
          ...
          if (targetMode.includes('己方') || targetMode.includes('友方')) {
            ...
            if (
              ...
              Number(getPrimaryDamageEffect(skill)?.防御穿透 || 0) >= 15 ||
        ...
        const skillTargetText = String(
          getSkillTarget(skill) ||
            ...
            (Array.isArray(skill._效果数组)
              ? skill._效果数组.map(effect => `${effect?.目标 || ''}`).join(' ')
              : ''),
        );
      ...
      function resolvePassivePlayerStance(playerAction, npcAction, 玩家单位 = null, NPC单位 = null) {
        ...
        const npcIsHostile =
          ...
          npcSkillType === '输出' ||
          npcMainType === '伤害类' ||
          hasSkillMechanism(npcSkill, ['伤害结算', '延迟结算']);
      ...
      function onPlayerAttack(playerInput, options = {}) {
        ...
        while (
          ...
          const playerInterruptChance = Math.max(
            Number(getSkillEffects(playerAction.skill).find(e => ['行动打断', '打断'].includes(String(e?.运行机制 || e?.原型 || '').trim()))?.中断概率 || 0),
      ...
      function getFusionScaledInterruptChance(skill = {}) {
        const interruptEffect = getSkillEffects(skill).find(effect => 读取战斗效果标签(effect) === '打断') || {};
      ...
      function ensureEffectTriggerLedger(char) {
        if (!char || typeof char !== 'object') return {};
        if (!char.效果触发记录 || typeof char.效果触发记录 !== 'object') char.效果触发记录 = {};
        return char.效果触发记录;
      }
      ...
      function consumeDailyMechanismTrigger(char, mechanismKey = '', limitPerDay = 0, currentTick = null) {
        ...
        const ledger = ensureEffectTriggerLedger(char);
      ...
      function resolveCastInterruptOnDamage(
        ...
        const interruptEffect = getSkillEffects(attackAction?.skill).find(effect => 读取战斗效果标签(effect) === '打断') || {};
      ...
      function 执行行为防反结算(攻击方, 防反方, 原动作, 反应动作, 交锋结果, 伤害包, 战斗数据) {
        ...
        const 原降低闪避 = 目标.temp_dodge_penalty;
        ...
        try {
          ...
          else 目标.temp_dodge_penalty = 原降低闪避;
      ...
      function 计算行为出手承诺(动作 = {}) {
        ...
        if (效果列表.some(effect => ['多段伤害', '追击', '追击位移', '强制位移', '位移交换'].includes(读取战斗效果标签(effect))))
          承诺 += 0.08;
        if (效果列表.some(effect => 读取战斗效果标签(effect) === '自残换收益')) 承诺 += 0.1;
      ...
      function estimateIncomingActionThreat(attacker, defender, playerAction, combatData) {
        ...
        skill = 按技能掌控度缩放技能(skill, attacker);
        ...
        let actualDef = 计算穿透后防御值(Number(defenderFinalStat.def || defender.def || 1), pClash?.防御穿透 || 0, conditionArmorPen);
        ...
        if (/物理/.test(dmgType)) {
          projectedDamage =
            ...
            定位伤害倍率 *
            消耗加成系数;
        } else if (/能量|元素|魂力|神圣|毒素/.test(dmgType)) {
          projectedDamage =
            skillPower *
      ...
      function 触发行动前效果授予(角色 = {}, 技能 = {}, 敌方 = null) {
        ...
        const 授予条目 = Object.entries(角色.状态效果).filter(([, 状态]) => {
          ...
          return 状态.效果授予状态 === true || 状态.战斗效果?.mechanism_grant === true;
        });
        ...
        授予条目.forEach(([状态名, 状态]) => {
          ...
          const 可执行效果 = 原效果列表
            .filter(effect => effect && typeof effect === 'object')
            .filter(effect => 读取战斗效果标签(effect) !== '机制授予')
            .map(effect => {
              const next = deepClone(effect);
              const 机制名 = 读取战斗效果标签(next);
              ...
              if (目标语义 === '敌对') {
                ...
                next.目标 = '自身';
              } else if (['食用者', '使用者'].includes(String(next.目标 || '').trim())) {
                next.目标 = '自身';
              }
              return next;
          });
          const 需要敌方 = 可执行效果.some(effect => {
            ...
            return meta?.目标语义 === '敌对' || ['单体', '群体'].includes(目标文本);
          });
          if (需要敌方 && !敌方) return;
      ...
      function executeClash(playerAction, npcAction, combatData) {
        ...
        playerAction.skill = normalizeSkillData(
          playerAction.skill || {
            ...
            前摇: 10,
            _效果数组: [
              { 原型: '伤害结算', 目标: '单体', 威力倍率: 100, 伤害类型: '物理近战', 防御穿透: 0 },
            ],
          },
        ...
        const 选定目标名 = String(playerAction?.target_name || '').trim();
        const 条件分支目标 = findCombatUnitByName(combatData, 选定目标名) || defender;
        const 当前技能条件上下文 = { actor: attacker, caster: attacker, target: 条件分支目标, combatData };
        const getEffect = (sk, types) => getSkillEffects(sk, 当前技能条件上下文).find(e => {
          const 原型 = String(e?.原型 || '').trim();
          return types.includes(原型);
        }) || {};

        let pClash = getEffect(playerAction.skill, ['伤害结算']);
        let pState = getEffect(playerAction.skill, ['状态施加']);
        ...
        if (技能效果增幅结果.已增幅) {
          ...
          pClash = getEffect(playerAction.skill, ['伤害结算']);
          pState = getEffect(playerAction.skill, ['状态施加']);
          if (!pState || typeof pState !== 'object' || Array.isArray(pState)) pState = {};
          if (!pState.状态名称) pState.状态名称 = '无';
          if (!pState.特殊机制标识) pState.特殊机制标识 = '无';
          if (!pState.目标) pState.目标 = getSkillTarget(playerAction.skill);
        ...
        const actionEffects = getSkillEffects(playerAction.skill, 当前技能条件上下文);
        ...
        const directStatusRemoveEffects = actionEffects.filter(effect => String(effect?.原型 || '').trim() === '状态移除');
        const directCleanseEffect = actionEffects.find(effect => ['解控', '净化'].includes(读取战斗效果标签(effect))) || null;
        const directDispelEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '驱散增益') || null;
        const directStealBuffEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '窃取增益') || null;
        const directRevealEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '破隐') || null;
        const directTauntEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '嘲讽') || null;
        const directGuardEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '护卫') || null;
        const 是指定原型 = (effect, 原型名) => String(effect?.原型 || '').trim() === 原型名;
        const 原型数值 = effect => 读取战斗数值正负(effect?.数值);
        const 护盾削减原型列表 = actionEffects.filter(effect => 是指定原型(effect, '护盾变化') && 原型数值(effect) < 0);
        const 护盾自增原型列表 = actionEffects.filter(effect => {
          if (!是指定原型(effect, '护盾变化') || 原型数值(effect) <= 0) return false;
          return String(effect?.目标 || '').trim() === '自身';
        });
        const 存在原型窃盾组合 = 护盾削减原型列表.length > 0 && 护盾自增原型列表.length > 0;
        const 是原型窃盾自增 = effect => 存在原型窃盾组合 && 护盾自增原型列表.includes(effect);
        const directPrototypeShieldStealEffect = 存在原型窃盾组合 ? 护盾削减原型列表[0] : null;
        const directPrototypeShieldStealGainEffect = 存在原型窃盾组合 ? 护盾自增原型列表[0] : null;
        const directShieldBreakEffect = 护盾削减原型列表.find(effect => effect !== directPrototypeShieldStealEffect) || null;
        ...
        const directResourceBurnEffect = actionEffects.find(effect =>
          effect?.原型 === '状态施加' &&
          String(effect?.状态 || effect?.状态名称 || '').trim() === '资源燃烧'
        ) || null;
        ...
        const directLifeLinkEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '生命链接') || null;
        const directDotExtendEffect = actionEffects.find(effect => effect?.原型 === '状态时窗修正' && String(effect?.调整方式 || '').trim() === '延长') || null;
        const directDotCompressEffect = actionEffects.find(effect => effect?.原型 === '状态时窗修正' && String(effect?.调整方式 || '').trim() === '压缩') || null;
        const directDotSplitStoreEffect = actionEffects.find(effect => effect?.原型 === '拆层转存') || null;
        const directDotDetonateEffect = actionEffects.find(effect => effect?.原型 === '结算修正' && String(effect?.结算 || '').trim() === '持续伤害引爆') || null;
        ...
        const selfMirrorEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '自身也受影响') || null;
        const directRandomTargetEffect = actionEffects.find(effect =>
          String(effect?.原型 || '').trim() === '目标选择修正' &&
          String(effect?.选择 || '').trim() === '随机目标偏转',
        ) || null;
        const directSelfSacrificeEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '自残换收益') || null;
        const directRingBurstEffect = actionEffects.find(effect => 读取战斗效果标签(effect) === '炸环') || null;
        const skillName = String(
          playerAction?.skill?.name || playerAction?.skill?.技能名称 || playerAction?.action_type || '',
        );
        const fusionProfile = getFusionBattleProfileFromSkill(playerAction.skill);
        const 引爆移除状态标记集 = new Set();
        const 原始技能效果列表 = Array.isArray(playerAction?.skill?._效果数组) ? playerAction.skill._效果数组 : [];
        const 效果目标包含自身 = effect =>
          resolveSkillTargetContext(playerAction.skill, attacker, defender, combatData, effect).targetSet.some(targetObj =>
            targetObj === attacker || isCombatUnitIdentityMatch(targetObj, attacker?.name || attacker),
          );
        const 条件分支禁用自身效果 = effect => {
          if (!Array.isArray(effect?.条件分支) || effect.条件分支.length <= 0) return false;
          if (!效果目标包含自身(effect)) return false;
          return 展开条件分支效果(effect, 当前技能条件上下文).length <= 0;
        };
        const 能力共享自身预禁用 = false;
        const 资源变化自身预禁用 = 原始技能效果列表.some(effect =>
          String(effect?.原型 || '').trim() === '资源变化' &&
          读取战斗数值正负(effect?.数值) > 0 &&
          条件分支禁用自身效果(effect),
        );
        if (能力共享自身预禁用 || 资源变化自身预禁用) attacker.__禁用本回合自然恢复 = true;
        ...
        const mirrorEffectToSelf = effect => (effect ? { ...effect, 目标: '自身' } : null);
        ...
        const resolveProjectedDamageAgainstTarget = (targetObj, index) => {
          ...
          let actualDef = 计算穿透后防御值(Number(targetFinalStat.def || targetObj.def || 1), pClash.防御穿透 || 0, conditionArmorPen);
          ...
          if (
            ...
            String(directExecuteEffect?.结算 || '').trim() === '最终伤害' &&
            getCombatHpValue(targetObj) / Math.max(1, getCombatHpMaxValue(targetObj)) <= 0.45
          ) {
            const execMult = 1 + Math.max(0, 读取战斗效果数值(directExecuteEffect?.数值, 0));
            if (execMult !== 1) {
              projectedDamage *= execMult;
              localLogParts.push(`[斩杀补伤] ${targetName}跌入收割阈值，追加杀伤生效。`);
            }
          }
        ...
        const resolveDirectMechanismTargetList = (effect, options = {}) => {
          ...
          if (
            ...
            const pickTarget = contextHint === '自身' ? friendlyTarget : hostileTarget || friendlyTarget;
          ...
          if (String(effect?.目标 || '').trim() === '自身' && (!targets.length || targets.every(targetObj => !targetObj))) {
        ...
        const applyImmediateRecoveryEffect = (effect, resourceKey, labelText) => {
          ...
          targetUnits.forEach(targetObj => {
            ...
            if (!amount) return;
            const convertToDamage = 读取战斗效果标签(resolvedEffect) === '伤害结算';
            if (convertToDamage) {
              const damage = Math.max(1, Math.floor(amount));
              设置战斗血量值(targetObj, getCombatHpValue(targetObj) - damage);
              totalRecovered += damage;
              result.desc += ` [条件分支] ${targetObj === attacker ? '自身' : targetObj.name}的${labelText}效果转化为 ${damage} 点伤害。`;
              return;
            }
        ...
        const applyShieldEffect = effect => {
          ...
          targetUnits.forEach(targetObj => {
            ...
            const rawShield = effect.护盾值 ?? effect.数值 ?? 0;
            const shieldValue = 读取战斗数值正负(rawShield);
            const targetMax = targetObj ? getCombatHpMaxValue(targetObj) : 0;
            const baseShield = /%$/.test(String(rawShield ?? '').trim()) || Math.abs(shieldValue) <= 1
              ? Math.floor(targetMax * Math.abs(shieldValue))
              : Math.floor(Math.abs(shieldValue));
        ...
        const applyRingBurstEffect = effect => {
          ...
          const 路径去重映射 = new Map();
          const 记录路径 = 路径 => {
            if (!Array.isArray(路径) || !路径.length) return;
            const 标准路径 = 路径.map(片段 => String(片段));
            const 键 = 标准路径.join('>');
            if (!键 || 路径去重映射.has(键)) return;
            路径去重映射.set(键, 标准路径);
          };
          记录路径(魂环路径);
          const 额外炸环路径来源 = [
            ...(Array.isArray(playerAction?.__炸环选择路径列表) ? playerAction.__炸环选择路径列表 : []),
            ...(Array.isArray(playerAction?.skill?.__炸环选择路径列表) ? playerAction.skill.__炸环选择路径列表 : []),
          ];
          额外炸环路径来源.forEach(记录路径);
          const 生效炸环路径列表 = Array.from(路径去重映射.values());
          const 不可用路径 = [];
          生效炸环路径列表.forEach(路径 => {
            const 魂环对象 = 按路径读取对象_V1(attacker, 路径);
            if (!魂环对象 || typeof 魂环对象 !== 'object') {
              不可用路径.push({ 路径, 原因: '路径无效' });
              return;
            }
            const 恢复检查 = 读取魂环路径恢复标记_V1(attacker, 路径);
            if (恢复检查.恢复中) {
              不可用路径.push({
                路径,
                原因: `恢复中(剩余${Math.max(1, Math.floor(Number(恢复检查.剩余tick || 0)))}tick)`,
              });
            }
          });
          if (不可用路径.length) {
            const 首条 = 不可用路径[0];
            result.desc += ` [炸环失败] 指定魂环不可用：${String(首条?.路径?.[首条.路径.length - 1] || '未知魂环')} ${首条.原因}。`;
            return false;
          }
          const 当前tick = 读取当前世界tick_V1();
          ...
          const 已炸魂环列表 = [];
          for (const 路径 of 生效炸环路径列表) {
            const 写回成功 = 写入角色魂环恢复标记_V1(attacker, 路径, 恢复截止tick, 恢复时间文本);
            if (!写回成功) {
              result.desc += ` [炸环失败] 魂环路径写回失败：${String(路径?.[路径.length - 1] || '未知魂环')}。`;
              return false;
            }
            if (角色名) {
              result.extraPatchOps.push(...构建魂环恢复标记补丁_V1(角色名, 路径, 恢复截止tick, 恢复时间文本));
            }
            const 魂环对象 = 按路径读取对象_V1(attacker, 路径);
            const 年限 = Math.max(100, Number(魂环对象?.年限 || 100));
            已炸魂环列表.push({
              路径,
              年限,
              档位类型: 判定魂环档位类型_V1(年限),
              魂环键: String(路径?.[路径.length - 1] || ''),
            });
          }
          if (!attacker.状态效果) attacker.状态效果 = {};
          const 年限增幅系数 = Math.max(0.001, Number(effect?.年限增幅系数 || 0.01));
          const 总年限贡献 = 已炸魂环列表.reduce((总和, 项) => 总和 + 计算炸环等效级差_V1(项.年限), 0);
          const 额外倍率 = Math.max(1.005, Number((1 + 年限增幅系数 * 总年限贡献).toFixed(4)));
          const 计算字段模板 = createEmptyCombatEffectMap();
          const 禁止字段正则 = /(次数|段数|触发次数|攻击段数)/;
          ...
          const 炸环明细文本 = 已炸魂环列表
            .map(项 => `${项.魂环键 || '魂环'}(${项.档位类型}/${Math.floor(Number(项.年限 || 0))}年)`)
            .join('、');
          result.desc += ` [炸环] 已炸${已炸魂环列表.length}枚魂环：${炸环明细文本}。预计于${恢复时间文本}恢复。下次技能增幅倍率 x${额外倍率.toFixed(2)}。`;
          return true;
        };
        ...
        const applyStatusTransferEffect = effect => {
          ...
          for (let index = 0; index < transferCount; index += 1) {
            ...
            for (const plan of 构建转移方案()) {
              ...
              const nextKey = putConditionWithUniqueKey(receiver, `${candidate.key}${plan.keySuffix || ''}`, moved);
              transferLogs.push(
        ...
        const applyDotDurationAdjustEffect = effect => {
          if (!effect) return false;
          const 调整方式 = String(effect?.调整方式 || '').trim() === '压缩' ? '压缩' : '延长';
          const targetUnits = resolveDirectMechanismTargetList(effect);
          ...
          if (changed) {
            if (调整方式 === '延长') result.desc += ` [持续调制] 目标持续伤害时窗被延长。`;
            if (调整方式 === '压缩') result.desc += ` [持续压缩] 已将部分持续伤害折算为即时打击。`;
          }
          return changed;
        };
        ...
        const applyShieldBreakEffect = effect => {
          if (!effect) return 0;
          const targetUnits = resolveDirectMechanismTargetList(effect);
          const prototypeValue = 读取战斗数值正负(effect?.数值);
          const ratio = Math.max(0, Math.abs(prototypeValue) || Number(effect?.斩盾倍率 || effect?.shield_break_ratio || 1));
          let totalAddedDamage = 0;
          targetUnits.forEach(targetObj => {
        ...
        const applyStealShieldEffect = effect => {
          ...
          const ratio = Math.max(0, Number(effect?.窃盾比例 || effect?.shield_steal_ratio || prototypeRatio || 0.5));
          const duration = Math.max(1, Number(effect?.持续回合 || directPrototypeShieldStealGainEffect?.持续回合 || 1));
          let totalShield = 0;
          targetUnits.forEach(targetObj => {
            const shieldEntries = collectShieldConditionEntries(targetObj);
            if (!shieldEntries.length) return;
            shieldEntries.forEach(entry => {
              ...
              applyShieldToCharacter(attacker, stolen, duration, `${skillName || '技能'}窃盾`);
              totalShield += stolen;
        ...
        const resolveTransferResourceKeys = rawType => {
          const source = Array.isArray(rawType) ? rawType : String(rawType || '').split(/[、,，/|｜；;\s]+/g);
          const text = source.map(item => String(item || '').trim()).filter(Boolean).join('、');
          const keys = [];
          if (/生命|体力|HP/i.test(text)) keys.push('vit');
          if (/魂力|sp/i.test(text) || /双|混合|全部/.test(text)) keys.push('sp');
          if (/精神|men/i.test(text) || /双|混合|全部/.test(text)) keys.push('men');
          return keys.length ? Array.from(new Set(keys)) : ['sp'];
        };

        const getTransferResourceLabel = resourceKey => {
          if (resourceKey === 'vit') return 'HP';
          if (resourceKey === 'men') return '精神力';
          return '魂力';
        };

        const 计算资源变化量 = (rawValue, maxValue) => {
          const text = String(rawValue ?? '').trim();
          const parsed = 读取战斗数值正负(text);
          if (!Number.isFinite(parsed) || parsed === 0) return 0;
          const amount = /%$/.test(text) || Math.abs(parsed) <= 1
            ? Math.floor(Math.max(1, Number(maxValue || 0)) * Math.abs(parsed))
        ...
        const applyResourceDrainEffect = effect => {
          if (!effect) return 0;
          const targetUnits = resolveDirectMechanismTargetList(effect);
          const ratio = Math.max(0, Math.abs(读取战斗数值正负(effect?.数值 ?? effect?.夺取比例 ?? effect?.drain_ratio ?? 0.18)));
          const convertRatio = Math.max(0, Number(effect?.转化比例 || effect?.convert_ratio || 1));
          const resourceKeys = resolveTransferResourceKeys(effect?.资源 || effect?.资源类型 || effect?.resource_type || '');
          let totalRecovered = 0;
          targetUnits.forEach(targetObj => {
            resourceKeys.forEach(resourceKey => {
              const maxKey = resourceKey === 'sp' ? 'sp_max' : 'men_max';
              const targetCurrent = resourceKey === 'vit' ? getCombatHpValue(targetObj) : Math.max(0, Number(targetObj?.[resourceKey] || 0));
              const targetMax = resourceKey === 'vit' ? getCombatHpMaxValue(targetObj) : Math.max(1, Number(targetObj?.[maxKey] || 0));
              ...
              if (resourceKey === 'vit') 设置战斗血量值(targetObj, Math.max(0, targetCurrent - drainAmount));
              else targetObj[resourceKey] = Math.max(0, targetCurrent - drainAmount);
              if (resourceKey === 'vit') return;
        ...
        const applyResourceRefeedEffect = effect => {
          ...
          targetUnits.forEach(targetObj => {
            if (能力共享自身预禁用 && targetObj === attacker) {
              ...
              result.desc += ` [条件分支] 自身能力共享不生效。`;
              return;
            }
            const resolvedEffect = effect;
            const ratio = Math.max(0, 读取战斗数值正负(resolvedEffect?.数值 ?? resolvedEffect?.反灌比例 ?? resolvedEffect?.refeed_ratio ?? 0.2));
            if (!(ratio > 0)) return;
            const resourceKeys = resolveTransferResourceKeys(resolvedEffect?.资源 || resolvedEffect?.资源类型 || resolvedEffect?.resource_type || '');
            if (isMechanismSuppressionBlocking(targetObj, ['回复机制'])) {
            ...
            resourceKeys.forEach(resourceKey => {
              ...
              if (convertToDamage) {
                const damage = Math.max(1, Math.floor(gainAmount));
                设置战斗血量值(targetObj, getCombatHpValue(targetObj) - damage);
                totalRecovered += damage;
                result.desc += ` [条件分支] ${targetObj === attacker ? '自身' : targetObj.name}的能力共享被转化为 ${damage} 点伤害。`;
                return;
              }
        ...
        const applyResourceBurnEffect = effect => {
          ...
          targetUnits.forEach(targetObj => {
            resourceKeys.forEach(resourceKey => {
              const maxKey = resourceKey === 'sp' ? 'sp_max' : 'men_max';
              const maxVal = resourceKey === 'vit' ? getCombatHpMaxValue(targetObj) : Math.max(1, Number(targetObj?.[maxKey] || 0));
              const rawBurn = effect?.数值 ?? effect?.每回合燃烧 ?? effect?.burn_ratio ?? -burnRatio;
              const burnAmount = 计算资源变化量(rawBurn, maxVal);
              const beforeValue = resourceKey === 'vit' ? getCombatHpValue(targetObj) : Math.max(0, Number(targetObj?.[resourceKey] || 0));
              ...
              if (resourceKey === 'vit') 设置战斗血量值(targetObj, afterValue);
              else targetObj[resourceKey] = afterValue;
              totalBurned += actualBurn;
              const label = String(effect?.触发 || '').trim() === '每回合' ? '资源燃烧' : '资源扣除';
              result.desc += ` [${label}] ${targetObj === attacker ? '自身' : targetObj.name}损失 ${actualBurn} 点${getTransferResourceLabel(resourceKey)}。`;
            });
          });
          return totalBurned;
        };
        ...
        const applyResourceLockEffect = effect => {
          ...
          const targetUnits = resolveDirectMechanismTargetList(effect);
          const prototypeRatio = Math.abs(读取战斗数值正负(effect?.数值));
          const lockRatio = Math.max(0, Math.min(1, Number(effect?.锁定比例 || effect?.lock_ratio || prototypeRatio || 0.5)));
          let applied = false;
          targetUnits.forEach(targetObj => {
            if (!targetObj?.状态效果) targetObj.状态效果 = {};
            targetObj.状态效果['资源锁定'] = {
        ...
        const applyMechanismSuppressEffect = effect => {
          ...
          const tags = normalizeBattleMechanismSuppressionTargets(effect?.抹消目标 || []);
          ...
          targetUnits.forEach(targetObj => {
            ...
            const stateKey = putConditionWithUniqueKey(targetObj, `机制抹消:${tags.join('/')}`, {
              ...
              抹消目标: tags,
              抹消方式: normalizeBattleMechanismSuppressionMode(effect?.抹消方式 || '移除并封锁'),
              战斗效果: { ...createEmptyCombatEffectMap() },
            });
        ...
        const runDirectMechanismConsumer = effect => {
          if (!effect) return false;
          if (effect?.原型 === '资源变化' && 读取战斗数值正负(effect?.数值) < 0) return applyResourceBurnEffect(effect);
          if (effect === directPrototypeShieldStealEffect) return applyStealShieldEffect(effect);
          if (effect?.原型 === '护盾变化' && 读取战斗数值正负(effect?.数值) < 0) return applyShieldBreakEffect(effect);
          ...
          if (effect?.原型 === '状态施加' && String(effect?.状态 || effect?.状态名称 || '').trim() === '资源燃烧') return false;
          ...
          const 运行时消费器 = String(getBattleSkillMechanismMeta(effect?.原型 || '')?.运行时消费器 || '').trim();
        ...
        if (directMenEffect) {
          ...
          if (directStatusRemoveEffects.length) {
            directStatusRemoveEffects.forEach(effect => {
              const statusTargets = resolveSkillEffectTargetCharacters(
                playerAction.skill,
                effect,
                attacker,
                defender,
              ...
              statusTargets.forEach(statusTarget => {
                const removed = removeConditionsByPrototypeFilter(statusTarget, effect);
                if (removed.length > 0)
                  result.desc += ` [状态移除] ${statusTarget === attacker ? '自身' : statusTarget.name}移除了[${removed.join('/')}]。`;
                else
                  result.desc += ` [状态移除] ${statusTarget === attacker ? '自身' : statusTarget.name}没有匹配状态。`;
              });
            });
          }
          ...
          if (
            ...
            stateTargets.forEach(targetObj => {
              ...
              targetObj.状态效果[pState.状态名称] = {
                ...
                抹消目标: normalizeBattleMechanismSuppressionTargets(pState.抹消目标 || []),
                抹消方式: normalizeBattleMechanismSuppressionMode(pState.抹消方式 || ''),
                面板修改比例: scaledMods,
                战斗效果: {
                  ...createEmptyCombatEffectMap(),
              ...
              if (targetObj.状态效果[pState.状态名称].抹消目标.length) {
                const removedStates = /移除/.test(targetObj.状态效果[pState.状态名称].抹消方式)
                  ? removeSuppressedMechanismStates(
                      targetObj,
                      targetObj.状态效果[pState.状态名称].抹消目标,
        ...
        function buildNpcSkillCandidateContext(
          ...
          function pickSkillWithWeight(skills) {
            const weighted = (skills || []).map(skill => {
              ...
              const 技能来源 = String(getSkillRuntimeMeta(skill).来源分类 || '魂技').trim() || '魂技';
              const 目标模型 = getSkillTargetModel(skill);
              const enemyHpRatio = getCombatHpRatio(attacker);
              ...
              const hasResourceRecover = getSkillEffects(skill, { 行为规划: true }).some(effect =>
                isBattleRecoverEffect(effect, ['sp', 'men']),
              ) || hasResourceRefeed;
              const hasHeal = getSkillEffects(skill, { 行为规划: true }).some(effect => isBattleRecoverEffect(effect, ['vit']));
              ...
              const penetrationValue = Number(getPrimaryDamageEffect(skill)?.防御穿透 || 0);
          ...
          const pierceSkill = 按画像取技能(skill => {
              ...
              return Number(dmg?.防御穿透 || 0) >= 15 || /破甲|穿透|粉碎/.test(String(skill?.name || ''));
          });
      ...
      // 📍 NPC 决策逻辑 (真实读取版)
        ...
        function buildPlayerActionFromSerializedEntry(entry) {
          ...
          const nextAction = {
            ...
            cast_time: Number(entry.前摇 ?? entry.cast_time ?? rawSkill.前摇 ?? rawSkill.cast_time ?? 10) || 10,
          ...
          if (Array.isArray(entry.__炸环选择路径列表)) {
            nextAction.__炸环选择路径列表 = entry.__炸环选择路径列表
              .filter(路径 => Array.isArray(路径) && 路径.length)
              .map(路径 => 路径.map(片段 => String(片段)));
            nextAction.skill.__炸环选择路径列表 = nextAction.__炸环选择路径列表.map(路径 => [...路径]);
          }
          if (entry.__批量消耗模式) {
        ...
        function 动作允许同名连放(单位 = {}, 动作 = {}) {
          ...
          if (/物品类型":"食物|食物/.test(技能文本)) return true;
        ...
        function 读取动作去重目标键(动作 = {}) {
          ...
          return String(动作.target_name || 动作.skill?.目标 || '默认目标').trim() || '默认目标';
        }
        ...
        function scoreEnemyTargetForSkill(attackerChar, target, skill) {
          ...
          const penetration = Number(dmg?.防御穿透 || 0);
            ...
            effectiveTargets.map(target => {
              ...
              if (focusTarget && target.name === focusTarget.name) {
                ...
                if (summary.控制强度 !== '无') weight -= 12;
                  ...
                  if (Number(getPrimaryDamageEffect(skill)?.防御穿透 || 0) >= 15 || summary.爆发级别 === '高')
                    weight += 14;
        ...
        function scoreAllyTargetForSkill(actorChar, ally, skill) {
          ...
          if (
            getSkillEffects(skill, { 行为规划: true }).some(effect => isBattleRecoverEffect(effect, ['vit'])) ||
            ['护盾', '免死', '护卫', '复苏', '减伤', '格挡', '霸体', '无敌', '替身', '分摊', '反射'].includes(summary.防御性质)
          )
            weight += Math.floor((1 - hpRatio) * 120) + (snapshot.hasBadCondition ? 15 : 0);
          if (getSkillEffects(skill, { 行为规划: true }).some(effect => isBattleRecoverEffect(effect, ['sp', 'men'])))
        ...
        function 准备自动批量辅助动作(actorChar, action, allyTeam = [], battleState = null) {
          ...
          const 允许批量 =
            ...
            getSkillEffects(action.skill, { 行为规划: true }).some(effect => isBattleRecoverEffect(effect));
        ...
        function runActorTurn(actorEntry, battleState) {
          ...
          if (reactionAction.type === '穿戴装备') {
            ...
            const actorInterruptChance = Math.max(
              Number(getSkillEffects(action.skill).find(e => ['行动打断', '打断'].includes(String(e?.运行机制 || e?.原型 || '').trim()))?.中断概率 || 0),
        ...
        function ui_getAvailableActions(charData, combatData) {
          ...
          availableSkills.forEach(skill => {
            ...
            const 技能来源 = String(getBattleSkillSourceCategory(skill) || skill.source_tag || '魂技').trim() || '魂技';
            const 技能动作 = {
        ...
        function pushUiSkillAction(actions, skill, fallbackName, source, 魂环路径 = null) {
          ...
          const runtimeSkill = normalizeSkillData(skill, name);
          if (Array.isArray(魂环路径) && 魂环路径.length) runtimeSkill.__魂环路径 = [...魂环路径];
          const 技能来源 = String(getBattleSkillSourceCategory(skill) || source || '魂技').trim() || '魂技';
          actions.push({
        ...
        function 获取动作炸环机制效果(action = {}) {
          ...
          return 效果列表.find(效果 => 效果 && String(效果.原型 || '').trim() === '结算修正' && String(效果.结算 || '').trim() === '炸环增幅') || null;
        }
        ...
        function buildIntentSerializedSkill(skill = {}) {
          const serialized = deepClonePlain(skill && typeof skill === 'object' ? skill : {});
          if (!serialized.魂技名 && serialized.name) serialized.魂技名 = serialized.name;
          if (serialized.前摇 === undefined && Number(skill?.cast_time ?? 0) >= 0) serialized.前摇 = Number(skill.cast_time || 0);
          [
            'cast_time',
            '__attributeCoeffBase',
            '战斗摘要',
            '战斗语义',
            'source_tag',
            '技能来源',
